...
MyAssistant はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/myassistant/releases) から直接ダウンロードするか、[myassistant.ai/download](https://myassistant.ai/download) を利用してください。
...
MyAssistant には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://myassistant.ai/docs/agents) の詳細はこちら。

### ドキュメント

MyAssistant の設定については [**ドキュメント**](https://myassistant.ai/docs) を参照してください。

### コントリビュート

MyAssistant に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。
...
- 100% オープンソース
- 特定のプロバイダーに依存しません。[MyAssistant Zen](https://myassistant.ai/zen) で提供しているモデルを推奨しますが、MyAssistant は Claude、OpenAI、Google、またはローカルモデルでも利用できます。モデルが進化すると差は縮まり価格も下がるため、provider-agnostic であることが重要です。
- そのまま使える LSP サポート
- TUI にフォーカス。MyAssistant は neovim ユーザーと [terminal.shop](https://terminal.shop) の制作者によって作られており、ターミナルで可能なことの限界を押し広げます。
- クライアント/サーバー構成。例えば MyAssistant をあなたのPCで動かし、モバイルアプリからリモート操作できます。TUI フロントエンドは複数あるクライアントの1つにすぎません。
