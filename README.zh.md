...
MyAssistant 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/anomalyco/myassistant/releases) 或 [myassistant.ai/download](https://myassistant.ai/download) 下载。
...
MyAssistant 内置两种 Agent，可用 `Tab` 键快速切换：
...
了解更多 [Agents](https://myassistant.ai/docs/agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://myassistant.ai/docs)。
...
- 100% 开源。
- 不绑定特定提供商。推荐使用 [MyAssistant Zen](https://myassistant.ai/zen) 的模型，但也可搭配 Claude、OpenAI、Google 甚至本地模型。模型迭代会缩小差异、降低成本，因此保持 provider-agnostic 很重要。
- 内置 LSP 支持。
- 聚焦终端界面 (TUI)。MyAssistant 由 Neovim 爱好者和 [terminal.shop](https://terminal.shop) 的创建者打造，会持续探索终端的极限。
