...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("myassistant-find")
  api.delete("myassistant-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("myassistant-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("myassistant-find", new Highlight(...rest))
    return true
  }
