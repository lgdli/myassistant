...
- Author package entrypoint is `@myassistant-ai/plugin/tui`.
...
- Import types from `@myassistant-ai/plugin/tui`.
- `@myassistant-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@myassistant-ai/plugin/tui"
...
That is what makes local config-scoped plugins able to import `@myassistant-ai/plugin/tui`.
