...
- Instance context is provided by `httpapi/server.ts` using `directory`, `workspace`, and `x-myassistant-directory`.
...
8. Run `bun typecheck` from `packages/myassistant`, relevant `bun run test:ci ...` tests from `packages/myassistant`, and `./packages/sdk/js/script/build.ts` from the repo root.
...
- Cover `directory` and `x-myassistant-directory` instance selection.
