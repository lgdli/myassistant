...
Practical reference for extracting a future `packages/server` from the current `packages/myassistant` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all myassistant services, Effect-first source of truth
- `packages/server` - myassistant server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full myassistant server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/myassistant`.
...
The intended direction here is the same, but the current `myassistant` package split is earlier in the migration.
...
- the current server host still lives in `packages/myassistant`
...
- if `packages/server` imports services from `packages/myassistant` while `packages/myassistant` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/myassistant` as the temporary Hono host
...
- `packages/server` must not import from `packages/myassistant`

Allowed in phase 1:

- `packages/myassistant` imports `packages/server`
...
- `packages/myassistant` shrinks or disappears as package responsibilities are fully split
...
- `packages/myassistant` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/myassistant`
...
Keep in `packages/myassistant` for now:
...
- host mounting from `packages/myassistant`
...
- no imports from `packages/myassistant` inside `packages/server`
...
- `packages/server` must still not import from `packages/myassistant`
...
### PR 4. Mount `packages/server` question from `packages/myassistant`

Scope:

- replace local experimental question route wiring in `packages/myassistant`
...
- mount it in parallel from `packages/myassistant`
...
- move adapter selection and server startup out of `packages/myassistant`

Rules:

- do not start this while `packages/server` still depends on `packages/myassistant`
...
- host logic no longer lives in `packages/myassistant`
...
3. the route is mounted from the current host in `packages/myassistant`
...
- `packages/server` imports services or runtime from `packages/myassistant`
- `packages/myassistant` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/myassistant` imports
...
- [ ] mount `question` from `packages/myassistant`
...
2. keep `packages/myassistant` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/myassistant`, stop and restructure the boundary first.
