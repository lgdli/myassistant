            ...
            get: () =>
              Effect.succeed({
                ...
                plugin_origins: plugins.map((plugin) => ({
                  spec: plugin,
                  source: path.join(directory, "myassistant.json"),
...
describe("plugin.auth-override", () => {
  test("user plugin overrides built-in github-copilot auth", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const pluginDir = path.join(dir, ".myassistant", "plugin")
    ...
    const plugin = pathToFileURL(path.join(tmp.path, ".myassistant", "plugin", "custom-copilot-auth.ts")).href
