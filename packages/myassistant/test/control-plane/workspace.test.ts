...
const originalWorkspacesFlag = Flag.MYASSISTANT_EXPERIMENTAL_WORKSPACES
const originalEnv = {
  MYASSISTANT_AUTH_CONTENT: process.env.MYASSISTANT_AUTH_CONTENT,
...
afterEach(async () => {
  ...
  Flag.MYASSISTANT_EXPERIMENTAL_WORKSPACES = originalWorkspacesFlag
  restoreEnv()
  await resetDatabase()
})
...
describe("workspace-old CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (dir) => {
      ...
      expect(JSON.parse(recorded.calls.create[0].env.MYASSISTANT_AUTH_CONTENT ?? "{}")).toEqual({
        test: { type: "api", key: "secret" },
      })
      expect(recorded.calls.create[0].env.MYASSISTANT_WORKSPACE_ID).toBe(workspaceID)
