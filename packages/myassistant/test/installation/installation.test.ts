...
describe("installation", () => {
  describe("latest", () => {
    ...
    test("reads npm versions via registry", async () => {
      ...
      expect(calls).toContain(`https://registry.npmjs.org/myassistant-ai/${InstallationChannel}`)
    })
    ...
    test("reads brew formulae API versions", async () => {
      const layer = testLayer(
        ...
        (cmd, args) => {
          ...
          if (cmd === "brew" && args.includes("--formula") && args.includes("anomalyco/tap/myassistant")) return ""
          if (cmd === "brew" && args.includes("--formula") && args.includes("myassistant")) return "myassistant"
          return ""
        },
      )
    ...
    test("reads brew tap info JSON via CLI", async () => {
      ...
      const layer = testLayer(
        ...
        (cmd, args) => {
          if (cmd === "brew" && args.includes("anomalyco/tap/myassistant") && args.includes("--formula")) return "myassistant"
