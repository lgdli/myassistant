...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.MYASSISTANT_TEST ?? "")'), {
      env: {
        MYASSISTANT_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "MYASSISTANT_TEST_SHELL"], {
      shell: true,
      env: {
        MYASSISTANT_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("MYASSISTANT_TEST_SHELL=ok")
  })
