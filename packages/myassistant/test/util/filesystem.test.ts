...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "myassistant.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "myassistant.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "myassistant.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "myassistant.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["myassistant.json", "myassistant.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "myassistant.json"),
        path.join(tmp.path, "myassistant.jsonc"),
        path.join(project, "myassistant.json"),
        path.join(project, "myassistant.jsonc"),
      ])
    })
  })
