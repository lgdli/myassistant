...
import { EffectFlock } from "@myassistant-ai/core/util/effect-flock"
...
import { AppFileSystem } from "@myassistant-ai/core/filesystem"
import { Env } from "../../src/env"
import { provideTestInstance, provideTmpdirInstance } from "../fixture/fixture"
import { tmpdir } from "../fixture/fixture"
import { InstanceRuntime } from "@/project/instance-runtime"
import { CrossSpawnSpawner } from "@myassistant-ai/core/cross-spawn-spawner"
...
const managedConfigDir = process.env.MYASSISTANT_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "myassistant.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "myassistant.json") {
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "myassistant.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "myassistant.jsonc"),
        JSON.stringify({
          $schema: "https://myassistant.ai/config.json",
          shell: "bash",
          model: "test/model",
  ...
  try {
    ...
    const file = path.join(tmp.path, "myassistant.jsonc")
...
test("ignores legacy tui keys in myassistant config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://myassistant.ai/config.json",
  ...
  try {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "myassistant.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["MYASSISTANT_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { myassistant: { options: { apiKey: "{env:MYASSISTANT_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["myassistant"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["MYASSISTANT_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["MYASSISTANT_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "myassistant.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("loads config from .myassistant directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".myassistant")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".myassistant", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .myassistant/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".myassistant")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .myassistant/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".myassistant")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .myassistant/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".myassistant")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only MYASSISTANT_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.MYASSISTANT_CONFIG_DIR
  process.env.MYASSISTANT_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.MYASSISTANT_CONFIG_DIR
    else process.env.MYASSISTANT_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable MYASSISTANT_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "myassistant.json"),
        JSON.stringify({ $schema: "https://myassistant.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .myassistant config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".myassistant")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .myassistant config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "myassistant.json"),
        JSON.stringify({
          $schema: "https://myassistant.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .myassistant config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "myassistant.json"),
        JSON.stringify({
          $schema: "https://myassistant.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".myassistant")
      ...
      await Filesystem.write(
        path.join(local, "myassistant.json"),
        JSON.stringify({
          $schema: "https://myassistant.ai/config.json",
...
// Note: preload.ts sets MYASSISTANT_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .myassistant config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .myassistant directory config enables it
      const opencodeDir = path.join(dir, ".myassistant")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "myassistant.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/myassistant")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/myassistant")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "myassistant.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-myassistant@2.4.3", file)).toBe("oh-my-myassistant@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-myassistant@2.4.3", "file:///project/.myassistant/plugin/oh-my-myassistant.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.myassistant/plugin/demo.ts", "file:///project/.myassistant/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.myassistant/plugin/demo.ts"])
  })
...
describe("MYASSISTANT_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["MYASSISTANT_DISABLE_PROJECT_CONFIG"]
    process.env["MYASSISTANT_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["MYASSISTANT_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["MYASSISTANT_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .myassistant/ directories when flag is set", async () => {
    const originalEnv = process.env["MYASSISTANT_DISABLE_PROJECT_CONFIG"]
    process.env["MYASSISTANT_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .myassistant directory with a command
          const opencodeDir = path.join(dir, ".myassistant", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await WithInstance.provide({
        ...
        fn: async () => {
          ...
          // Project .myassistant should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["MYASSISTANT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MYASSISTANT_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["MYASSISTANT_CONFIG_DIR"]
      process.env["MYASSISTANT_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["MYASSISTANT_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["MYASSISTANT_CONFIG_DIR"]
      } else {
        process.env["MYASSISTANT_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("MYASSISTANT_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["MYASSISTANT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MYASSISTANT_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["MYASSISTANT_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["MYASSISTANT_CONFIG_DIR"] = configDirTmp.path

      await WithInstance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from MYASSISTANT_CONFIG_DIR, not project
...
describe("MYASSISTANT_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in MYASSISTANT_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MYASSISTANT_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["MYASSISTANT_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://myassistant.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["MYASSISTANT_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["MYASSISTANT_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in MYASSISTANT_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MYASSISTANT_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.effectSchema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "MyAssistant Managed",
          PayloadIdentifier: "ai.myassistant.managed.test",
          PayloadType: "ai.myassistant.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://myassistant.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://myassistant.ai/config.json")
})
