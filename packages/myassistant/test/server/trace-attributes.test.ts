...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under myassistant.", () => {
    expect(paramToAttributeKey("name")).toBe("myassistant.name")
    expect(paramToAttributeKey("slug")).toBe("myassistant.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["myassistant.sessionID"]).toBeUndefined()
    expect(attrs["myassistant.messageID"]).toBeUndefined()
    expect(attrs["myassistant.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under myassistant. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["myassistant.name"]).toBe("exa")
