...
const originalHttpApi = Flag.MYASSISTANT_EXPERIMENTAL_HTTPAPI
...
afterEach(async () => {
  Flag.MYASSISTANT_EXPERIMENTAL_HTTPAPI = originalHttpApi
...
describe("HttpApi raw route authorization", () => {
  test("requires configured auth before opening the raw instance event stream", async () => {
    ...
    const authed = await server.request(EventPaths.event, {
      headers: { ...headers, authorization: basic("myassistant", "secret") },
    })
