...
const original = {
  ...
  MYASSISTANT_DISABLE_EMBEDDED_WEB_UI: Flag.MYASSISTANT_DISABLE_EMBEDDED_WEB_UI,
...
afterEach(() => {
  ...
  Flag.MYASSISTANT_DISABLE_EMBEDDED_WEB_UI = original.MYASSISTANT_DISABLE_EMBEDDED_WEB_UI
  ...
  restoreEnv("MYASSISTANT_SERVER_PASSWORD", original.envPassword)
  restoreEnv("MYASSISTANT_SERVER_USERNAME", original.envUsername)
})
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            MYASSISTANT_SERVER_PASSWORD: input?.password,
            MYASSISTANT_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  test("serves the web UI through the experimental backend", async () => {
    Flag.MYASSISTANT_EXPERIMENTAL_HTTPAPI = true
  ...
  test("strips upstream transfer encoding headers from proxied assets", async () => {
    ...
    expect(response.status).toBe(200)
    expect(proxiedUrl).toBe("https://app.myassistant.ai/assets/app.js")
  ...
  test("allows web UI preflight without auth", async () => {
    ...
    const response = await app({ password: "secret", username: "myassistant" }).request("/", {
      method: "OPTIONS",
      headers: {
