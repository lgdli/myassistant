...
function app(backend: Backend, input?: { password?: string; username?: string }) {
  Flag.MYASSISTANT_EXPERIMENTAL_HTTPAPI = backend === "httpapi"
  Flag.MYASSISTANT_SERVER_PASSWORD = input?.password
  Flag.MYASSISTANT_SERVER_USERNAME = input?.username
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("myassistant", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("myassistant", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
