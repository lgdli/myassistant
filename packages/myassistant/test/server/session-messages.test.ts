...
async function withoutWatcher<T>(fn: () => Promise<T>) {
  ...
  const prev = process.env.MYASSISTANT_EXPERIMENTAL_DISABLE_FILEWATCHER
  process.env.MYASSISTANT_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  try {
    return await fn()
  } finally {
    if (prev === undefined) delete process.env.MYASSISTANT_EXPERIMENTAL_DISABLE_FILEWATCHER
    else process.env.MYASSISTANT_EXPERIMENTAL_DISABLE_FILEWATCHER = prev
  }
}
