...
const original = Flag.MYASSISTANT_EXPERIMENTAL_HTTPAPI
...
afterEach(async () => {
  Flag.MYASSISTANT_EXPERIMENTAL_HTTPAPI = original
...
describe("instance HttpApi", () => {
  test("serves catalog read endpoints through Hono bridge", async () => {
    ...
    const [commands, agents, skills, lsp, formatter] = await Promise.all([
      app().request(InstancePaths.command, { headers: { "x-myassistant-directory": tmp.path } }),
      app().request(InstancePaths.agent, { headers: { "x-myassistant-directory": tmp.path } }),
      app().request(InstancePaths.skill, { headers: { "x-myassistant-directory": tmp.path } }),
      app().request(InstancePaths.lsp, { headers: { "x-myassistant-directory": tmp.path } }),
      app().request(InstancePaths.formatter, { headers: { "x-myassistant-directory": tmp.path } }),
    ])
  ...
  test("serves project git init through Hono bridge", async () => {
    ...
    const current = await app().request("/project/current", { headers: { "x-myassistant-directory": tmp.path } })
  ...
  test("serves project update through Hono bridge", async () => {
    ...
    const response = await app().request(`/project/${project.id}`, {
      method: "PATCH",
      headers: { "x-myassistant-directory": tmp.path, "content-type": "application/json" },
    ...
    const list = await app().request("/project", { headers: { "x-myassistant-directory": tmp.path } })
