...
const directoryHeader = (dir: string) => HttpClientRequest.setHeader("x-myassistant-directory", dir)
...
describe("pty HttpApi bridge", () => {
  test("serves available shell list through experimental Effect routes", async () => {
    ...
    const response = await app().request(PtyPaths.shells, { headers: { "x-myassistant-directory": tmp.path } })
