...
import type { Event, EventMessagePartUpdated, ToolStatePending, ToolStateRunning } from "@myassistant-ai/sdk/v2"
...
function createFakeAgent() {
  ...
  const agent = new ACP.Agent(connection, {
    sdk,
    defaultModel: { providerID: "myassistant", modelID: "big-pickle" },
  } as any)
  ...
  test("routes message.part.delta by the event sessionID (no cross-session pollution)", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const cwd = "/tmp/myassistant-acp-test"
