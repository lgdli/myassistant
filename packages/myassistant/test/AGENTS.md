...
- `config?: Partial<Config.Info>` - Write an `myassistant.json` config file
...
- Directories are created in the system temp folder with prefix `myassistant-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("myassistant-test-")
  }),
)
```
