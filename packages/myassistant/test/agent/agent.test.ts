...
test("plan agent denies edits except .myassistant/plans/*", async () => {
  await using tmp = await tmpdir()
  await WithInstance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".myassistant/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".myassistant", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  const home = process.env.MYASSISTANT_TEST_HOME
  process.env.MYASSISTANT_TEST_HOME = tmp.path
  ...
  try {
    await WithInstance.provide({
      ...
      fn: async () => {
        const build = await load(tmp.path, (svc) => svc.get("build"))
        const skillDir = path.join(tmp.path, ".myassistant", "skill", "perm-skill")
