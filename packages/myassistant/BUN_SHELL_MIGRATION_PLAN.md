...
Replace runtime Bun shell template-tag usage in `packages/myassistant/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/myassistant/src` is removed except:
