...
import { Flock } from "@myassistant-ai/core/util/flock"
...
export class Service extends Context.Service<Service, Interface>()("@myassistant/ModelsDev") {}
...
export const layer: Layer.Layer<Service, never, AppFileSystem.Service | HttpClient.HttpClient> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.MYASSISTANT_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.MYASSISTANT_MODELS_PATH ?? filepath).pipe(
    ...
    const populate = Effect.gen(function* () {
      ...
      if (Flag.MYASSISTANT_DISABLE_MODELS_FETCH) return {}
      // Flock is cross-process: concurrent myassistant CLIs can race on this cache file.
    ...
    if (!Flag.MYASSISTANT_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
