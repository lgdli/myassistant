...
const MANAGED_PLIST_DOMAIN = "ai.myassistant.managed"

// Keys injected by macOS/MDM into the managed plist that are not MyAssistant config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/myassistant"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "myassistant")
    default:
      return "/etc/myassistant"
  }
}

export function managedConfigDir() {
  return process.env.MYASSISTANT_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
