...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@myassistant-ai/sdk/v2"
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `myassistant auth login` in the terminal",
      name: "Login with myassistant",
      id: "myassistant-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // MyAssistant stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const myassistantProvider = providers.find((p) => p.id === "myassistant")
  if (myassistantProvider) {
    if (myassistantProvider.models["big-pickle"]) {
      return { providerID: ProviderID.myassistant, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(myassistantProvider.models))
    if (best) {
      return {
