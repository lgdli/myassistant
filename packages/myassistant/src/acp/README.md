...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for myassistant.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal myassistant sessions
...
myassistant acp --cwd /path/to/project
```
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 myassistant acp
```
...
- **Terminal Integration**: Full terminal support via myassistant's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | myassistant acp
```
...
### Mapping to MyAssistant

ACP sessions map cleanly to myassistant's internal session model:
