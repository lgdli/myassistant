...
export const DEPRECATED_PLUGIN_PACKAGES = ["myassistant-openai-codex-auth", "myassistant-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, myassistantVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(myassistantVersion) || semver.major(myassistantVersion) === 0) return
