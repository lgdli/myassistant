...
export type PatchDeps = {
  ...
  files: (dir: string, name: "myassistant" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".myassistant")
}

function patchName(kind: Kind): "myassistant" | "tui" {
  if (kind === "server") return "myassistant"
  return "tui"
}
