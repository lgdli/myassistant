...
import { MyAssistantHttpApi } from "./api"
...
const LegacyComponentDescriptions = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for myassistant serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw = OpenApi.fromApi(MyAssistantHttpApi) as unknown as OpenApiSpec
...
export const PublicApi = MyAssistantHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "myassistant",
    version: "1.0.0",
