...
function createHono(opts: CorsOptions, selection: ServerBackend.Selection = ServerBackend.force(select(), "hono")) {
  ...
  if (Flag.MYASSISTANT_WORKSPACE_ID) {
    return {
      app: app
        .use(InstanceMiddleware(Flag.MYASSISTANT_WORKSPACE_ID ? WorkspaceID.make(Flag.MYASSISTANT_WORKSPACE_ID) : undefined))
        .use(FenceMiddleware)
 ...
 * `PublicApi` is `MyAssistantHttpApi` annotated with the `matchLegacyOpenApi`
...
async function listenHttpApi(opts: ListenOptions, selection: ServerBackend.Selection): Promise<Listener> {
  log.info("server backend selected", {
    ...
    "myassistant.server.runtime": HttpApiServer.name,
  })
