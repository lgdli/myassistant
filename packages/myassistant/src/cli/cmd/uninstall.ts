...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall myassistant and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall MyAssistant")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g myassistant-ai",
      pnpm: "pnpm uninstall -g myassistant-ai",
      bun: "bun remove -g myassistant-ai",
      yarn: "yarn global remove myassistant-ai",
      brew: "brew uninstall myassistant",
      choco: "choco uninstall myassistant",
      scoop: "scoop uninstall myassistant",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "myassistant", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".myassistant")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using MyAssistant!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# myassistant") || content.includes(".myassistant/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".myassistant/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".myassistant/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".myassistant"))
    ) {
      continue
    }
