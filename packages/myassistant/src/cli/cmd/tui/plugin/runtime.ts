...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@myassistant-ai/plugin/tui"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".myassistant"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".myassistant", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".myassistant", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await WithInstance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.MYASSISTANT_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.MYASSISTANT_PURE && config.plugin_origins?.length) {
