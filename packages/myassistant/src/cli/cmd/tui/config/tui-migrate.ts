...
const TUI_SCHEMA_URL = "https://myassistant.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from myassistant.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const myassistant = await myassistantFiles(input)
  for (const file of myassistant) {
...
async function myassistantFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "myassistant"),
    ...(await Filesystem.findUp(["myassistant.json", "myassistant.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "myassistant"))
  }
  if (Flag.MYASSISTANT_CONFIG) files.push(Flag.MYASSISTANT_CONFIG)
