...
export class Service extends Context.Service<Service, Interface>()("@myassistant/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old myassistant.json shape) still get their settings applied.
  ...
  const projectFiles = Flag.MYASSISTANT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit MYASSISTANT_TUI_CONFIG override, if set.
  if (Flag.MYASSISTANT_TUI_CONFIG) {
    const configFile = Flag.MYASSISTANT_TUI_CONFIG
