 ...
 *
 * The script intentionally isolates `MYASSISTANT_DB` before importing modules that touch
...
const preserveExerciseGlobalRoot = !!process.env.MYASSISTANT_HTTPAPI_EXERCISE_GLOBAL
const exerciseGlobalRoot =
  process.env.MYASSISTANT_HTTPAPI_EXERCISE_GLOBAL ??
  path.join(process.env.TMPDIR ?? "/tmp", `myassistant-httpapi-global-${process.pid}`)
process.env.XDG_DATA_HOME = path.join(exerciseGlobalRoot, "data")
process.env.XDG_CONFIG_HOME = path.join(exerciseGlobalRoot, "config")
process.env.XDG_STATE_HOME = path.join(exerciseGlobalRoot, "state")
process.env.XDG_CACHE_HOME = path.join(exerciseGlobalRoot, "cache")
process.env.MYASSISTANT_DISABLE_SHARE = "true"
const exerciseConfigDirectory = path.join(exerciseGlobalRoot, "config", "myassistant")
const exerciseDataDirectory = path.join(exerciseGlobalRoot, "data", "myassistant")

const preserveExerciseDatabase = !!process.env.MYASSISTANT_HTTPAPI_EXERCISE_DB
const exerciseDatabasePath =
  process.env.MYASSISTANT_HTTPAPI_EXERCISE_DB ??
  path.join(process.env.TMPDIR ?? "/tmp", `myassistant-httpapi-exercise-${process.pid}.db`)
process.env.MYASSISTANT_DB = exerciseDatabasePath
Flag.MYASSISTANT_DB = exerciseDatabasePath

void (await import("@myassistant-ai/core/util/log")).init({ print: false })
...
const original = {
  MYASSISTANT_EXPERIMENTAL_HTTPAPI: Flag.MYASSISTANT_EXPERIMENTAL_HTTPAPI,
...
const scenarios: Scenario[] = [
  ...
  http
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "myassistant.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "myassistant.jsonc")).text(),
          )
  ...
  http.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-myassistant-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-myassistant-directory")
  }),
  ...
  http
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "myassistant", model: "test" })}`,
      headers: ctx.headers(),
    }))
    ...
    Effect.flatMap((context) =>
      Effect.gen(function* () {
        ...
        const base: ScenarioContext = {
          ...
          headers: (extra) => ({
            ...(context.dir?.path ? { "x-myassistant-directory": context.dir.path } : {}),
            ...extra,
          }),
          file: (name, content) =>
            Effect.promise(() => {
          ...
          message: (sessionID, input) =>
            Effect.gen(function* () {
              const info: MessageV2.User = {
                ...
                agent: "build",
                model: {
                  providerID: ProviderID.myassistant,
...
function app(modules: Runtime, backend: Backend) {
  Flag.MYASSISTANT_EXPERIMENTAL_HTTPAPI = backend === "effect"
    ...
    modules.ExperimentalHttpApiServer.routes.pipe(
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({ MYASSISTANT_SERVER_PASSWORD: undefined, MYASSISTANT_SERVER_USERNAME: undefined }),
        ),
      ),
    ),
    { disableLogger: true },
...
const resetState = Effect.promise(async () => {
  ...
  Flag.MYASSISTANT_EXPERIMENTAL_HTTPAPI = original.MYASSISTANT_EXPERIMENTAL_HTTPAPI
