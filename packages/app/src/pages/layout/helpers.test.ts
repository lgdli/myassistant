...
import { type Session } from "@myassistant-ai/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("myassistant://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("myassistant://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("myassistant://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("myassistant://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("myassistant://open-project")).toBeUndefined()
    expect(parseDeepLink("myassistant://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "myassistant://open-project?directory=/a",
      "myassistant://other?directory=/b",
      "myassistant://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("myassistant://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("myassistant://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("myassistant://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("myassistant://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "myassistant://new-session?directory=/a",
      "myassistant://open-project?directory=/b",
      "myassistant://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __MYASSISTANT__: {
        deepLinks: ["myassistant://open-project?directory=/a"],
      },
    } as unknown as Window & { __MYASSISTANT__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["myassistant://open-project?directory=/a"])
