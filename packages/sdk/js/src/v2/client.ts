...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-myassistant-directory", "directory"],
    ["x-myassistant-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-myassistant-directory")
  next.headers.delete("x-myassistant-workspace")
  return next
}
...
export function createOpencodeClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-myassistant-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-myassistant-workspace": config.experimental_workspaceID,
    }
  }
  ...
  client.interceptors.response.use((response) => {
    ...
    if (contentType === "text/html")
      throw new Error("Request is not supported by this version of MyAssistant Server (Server responded with text/html)")
  ...
  client.interceptors.error.use((error, response, request) => {
    ...
    if (!response) return new Error(`myassistant server ${method} ${url}: network error (no response)`)
    const status = response.status
    const statusText = response.statusText ? " " + response.statusText : ""
    return new Error(`myassistant server ${method} ${url} → ${status}${statusText}: (empty response body)`)
  })
