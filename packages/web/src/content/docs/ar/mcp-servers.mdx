...
يمكنك إضافة أدوات خارجية إلى MyAssistant باستخدام _Model Context Protocol_، أو MCP. يدعم MyAssistant خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات MyAssistant](https://myassistant.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/myassistant` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى MyAssistant تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم MyAssistant بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك MyAssistant المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `myassistant mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن MyAssistant الرموز بشكل آمن في `~/.local/share/myassistant/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل MyAssistant إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات MyAssistant مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط MyAssistant بحساب Sentry الخاص بك.
