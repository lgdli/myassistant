---
title: المزوّدون
description: استخدام أي موفّر LLM في MyAssistant.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم MyAssistant ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات MyAssistant.
...
في `~/.local/share/myassistant/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات MyAssistant.
...
MyAssistant Zen هي قائمة نماذج يوفّرها فريق MyAssistant وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع MyAssistant. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع MyAssistant Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر myassistant، ثم انتقل إلى [myassistant.ai/auth](https://myassistant.ai/auth).
...
يعمل مثل أي مزوّد آخر في MyAssistant واستخدامه اختياري بالكامل.

---

## MyAssistant Go

MyAssistant Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق MyAssistant، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع MyAssistant.

1. شغّل الأمر `/connect` في TUI، واختر `MyAssistant Go`، ثم انتقل إلى [myassistant.ai/auth](https://myassistant.ai/zen).
...
لاستخدام Amazon Bedrock مع MyAssistant:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل myassistant:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `myassistant.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل MyAssistant غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين myassistant لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل myassistant بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات myassistant.
...
تخزين الرمز في مساحة تخزين مصادقة myassistant.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم MyAssistant نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر MyAssistant
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`myassistant.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [myassistant-gitlab-auth](https://www.npmjs.com/package/myassistant-gitlab-auth).

##### التهيئة

خصّص عبر `myassistant.json`:
...
لاستخدام اشتراك GitHub Copilot مع myassistant:
...
لاستخدام Google Vertex AI مع MyAssistant:
...
2. عيّن متغيرات البيئة المطلوبة:
   ...
   عيّنها أثناء تشغيل myassistant.
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر myassistant، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [myassistant-helicone-session](https://github.com/H2Shami/myassistant-helicone-session) لتسجيل كل محادثة MyAssistant تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة MyAssistant مدرجة كجلسة مستقلة.
...
يمكنك ضبط myassistant لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط myassistant لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط myassistant لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع MyAssistant. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/myassistant) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع MyAssistant:
...
3. انسخ مفتاح API لاستخدامه في MyAssistant.
...
6. **مهم**: قبل استخدام نماذج السحابة في MyAssistant، يجب سحب معلومات النموذج محليا:
...
MyAssistant Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق MyAssistant. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>MyAssistant Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **MyAssistant Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ MyAssistant.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات myassistant.
...
5. يمكنك أيضا تخصيصها عبر إعدادات myassistant. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات myassistant. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع myassistant. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `myassistant.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ MyAssistant بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `myassistant auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات myassistant وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات myassistant.
