---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع MyAssistant.
---

تتيح لك الإضافات توسيع MyAssistant عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ MyAssistant.
...
- `.myassistant/plugins/` - إضافات على مستوى المشروع
- `~/.config/myassistant/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/myassistant/node_modules/`.
...
1. إعدادات عامة (`~/.config/myassistant/myassistant.json`)
2. إعدادات المشروع (`myassistant.json`)
3. دليل الإضافات العام (`~/.config/myassistant/plugins/`)
4. دليل إضافات المشروع (`.myassistant/plugins/`)
...
يشغّل MyAssistant الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ myassistant للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع myassistant.
...
:::note
إذا كنت تستخدم تطبيق MyAssistant لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع myassistant من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى myassistant:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ myassistant استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ myassistant إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://myassistant.ai/docs/sdk) للتفاصيل.
