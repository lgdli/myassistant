---
title: CLI
description: أوامر وخيارات CLI في MyAssistant.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ MyAssistant افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
myassistant
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع MyAssistant برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ MyAssistant.
...
يتضمن CLI الخاص بـ MyAssistant أيضا الأوامر التالية.

---

### agent

إدارة وكلاء MyAssistant.
...
إرفاق terminal بخادم الواجهة الخلفية لـ MyAssistant قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
myassistant attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ MyAssistant تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `myassistant`) |
...
يعتمد MyAssistant على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `myassistant auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/myassistant/auth.json`.

```bash
myassistant auth login
```

عند تشغيل MyAssistant يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في MyAssistant.
...
تشغيل myassistant في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `myassistant serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| <nobr><code>{"--attach"}</code></nobr>   |         | الإرفاق بخادم myassistant قيد التشغيل (مثل http://localhost:4096)                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `myassistant`) |
...
بدء خادم MyAssistant بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
myassistant serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف myassistant دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `myassistant`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات MyAssistant لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة MyAssistant.

```bash
myassistant import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة MyAssistant.
...
بدء خادم MyAssistant بدون واجهة مع واجهة ويب.

```bash
myassistant web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى MyAssistant عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `myassistant`).
...
إلغاء تثبيت MyAssistant وإزالة جميع الملفات ذات الصلة.
...
تحديث myassistant إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر myassistant الخيارات العامة التالية.
...
يمكن تهيئة MyAssistant باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `myassistant`) |
