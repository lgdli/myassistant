---
title: الخادم
description: تفاعل مع خادم myassistant عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `myassistant serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل myassistant استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `myassistant.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `myassistant`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `myassistant serve` و `myassistant web`.
...
عند تشغيل `myassistant` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم myassistant للتفاعل مع myassistant برمجيا.
:::

تتيح هذه البنية لـ myassistant دعم عدة عملاء وتمكّنك من التفاعل مع myassistant برمجيا.

يمكنك تشغيل `myassistant serve` لبدء خادم مستقل. إذا كانت واجهة myassistant في terminal (TUI)
قيد التشغيل، فسيبدأ `myassistant serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات MyAssistant لـ [IDE](/docs/ide).
...
يعرض خادم myassistant واجهات API التالية.
