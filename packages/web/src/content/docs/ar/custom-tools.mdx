...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في myassistant.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في myassistant مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.myassistant/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/myassistant/tools/`.
