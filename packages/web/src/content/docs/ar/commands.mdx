...
أنشئ الملف `.myassistant/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات MyAssistant أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ MyAssistant:
...
- على مستوى النظام: `~/.config/myassistant/commands/`
- لكل مشروع: `.myassistant/commands/`
...
يتضمن myassistant عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
