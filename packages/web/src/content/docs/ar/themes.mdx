...
مع MyAssistant يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم MyAssistant سمتنا `myassistant`.
...
يأتي MyAssistant مع عدة سمات مدمجة.
...
- يريدون أن يطابق MyAssistant مظهر terminal لديهم
...
يدعم MyAssistant نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/myassistant/themes/*.json` أو `$XDG_CONFIG_HOME/myassistant/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.myassistant/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.myassistant/themes/*.json`
