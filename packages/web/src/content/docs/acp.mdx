---
title: ACP Support
description: Use MyAssistant in any ACP-compatible editor.
---

MyAssistant supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use MyAssistant via ACP, configure your editor to run the `myassistant acp` command.

The command starts MyAssistant as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'MyAssistant' agent in the AI Chat agent selector.
...
To use MyAssistant as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use MyAssistant as the ACP agent for chat.
...
MyAssistant works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your MyAssistant config
