...
Dzięki myassistant możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie myassistant używa naszego własnego motywu `myassistant`.
...
myassistant ma kilka wbudowanych motywów.
...
- Chcą, aby MyAssistant pasował do wyglądu ich terminala
...
myassistant obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/myassistant/themes/*.json` lub `$XDG_CONFIG_HOME/myassistant/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.myassistant/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.myassistant/themes/*.json`
