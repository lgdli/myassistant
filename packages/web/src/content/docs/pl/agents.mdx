...
W MyAssistant obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
MyAssistant zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
MyAssistant ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

MyAssistant ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `myassistant.json`:
...
- Globalnie: `~/.config/myassistant/agents/`
- W projekcie: `.myassistant/agents/`
...
Jeśli nie zostanie skonfigurowana, MyAssistant używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji MyAssistant, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji MyAssistant w `provider/model-id`. Na przykład, [MyAssistant Zen](/docs/zen), użyj `myassistant/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `myassistant models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/myassistant).
:::
