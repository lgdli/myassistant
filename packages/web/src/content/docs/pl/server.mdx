---
title: Serwer
description: Interakcja z serwerem myassistant za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `myassistant serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient myassistant.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `myassistant.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `myassistant` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `myassistant serve`, jak i `myassistant web`.
...
Kiedy uruchomisz `myassistant`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera myassistant do programowej interakcji z myassistant.
:::

Ta architektura umożliwia obsługę wielu klientów przez myassistant i programową interakcję z kodem otwartym.

Możesz uruchomić `myassistant serve`, aby uruchomić samodzielny serwer. Jeśli masz
myassistant TUI działa, `myassistant serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki myassistant [IDE](/docs/ide).
...
Serwer myassistant udostępnia następujące interfejsy API.
