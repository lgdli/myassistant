---
title: CLI
description: Opcje i polecenia wiersza poleceń MyAssistant.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI MyAssistant domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
myassistant
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać MyAssistant programowo.
...
Uruchamia interfejs terminalowy (TUI) MyAssistant.
...
Interfejs CLI MyAssistant zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami MyAssistant.
...
Dołącz terminal do już działającego serwera backendu MyAssistant uruchomionego za pomocą `serve` lub `web`.

```bash
myassistant attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem MyAssistant. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `myassistant`) |
...
MyAssistant integruje się z wieloma dostawcami. Możesz użyć `myassistant auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/myassistant/auth.json`.

```bash
myassistant auth login
```

Podczas uruchamiania MyAssistant ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w MyAssistant.
...
Uruchom MyAssistant w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `myassistant serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera MyAssistant (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `myassistant`) |
...
Uruchom serwer MyAssistant (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
myassistant serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API MyAssistant bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `myassistant`).
...
Lista wszystkich sesji MyAssistant.
...
Pokaż statystyki wykorzystania tokenów i sesji MyAssistant.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania MyAssistant.
...
Uruchom serwer MyAssistant z interfejsem internetowym.

```bash
myassistant web
```

Uruchamia to serwer HTTP i udostępnia MyAssistant przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `myassistant`).
...
Odinstaluj MyAssistant i usuń wszystkie powiązane pliki.
...
Aktualizuj MyAssistant do najnowszej lub określonej wersji.
...
Interfejs CLI MyAssistant przyjmuje następujące flagi globalne dla każdego polecenia.
...
MyAssistant można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `myassistant`)    |
