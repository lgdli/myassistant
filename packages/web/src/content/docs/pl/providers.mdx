---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w myassistant.
---

import config from "../../../../config.mjs"
export const console = config.console

myassistant używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji myassistant.
...
w `~/.local/share/myassistant/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim myassistant
config.
...
MyAssistant Zen to lista modeli dostarczonych przez zespół myassistant, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z myassistant. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od MyAssistant Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `MyAssistant Zen` i przejdź do [myassistant.ai/auth](https://myassistant.ai/zen).
...
Działa jak każdy inny dostawca w myassistant i jest całkowicie opcjonalny w użyciu.

---

## MyAssistant Go

MyAssistant Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół myassistant, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z myassistant.

1. Uruchom polecenie `/connect` w TUI, wybierz `MyAssistant Go` i przejdź do [myassistant.ai/auth](https://myassistant.ai/zen).
...
Aby używać Amazon Bedrock z myassistant:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania myassistant:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `myassistant.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w myassistant nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować myassistant do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod myassistant działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji myassistant.
...
myassistant używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować myassistant
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`myassistant.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [myassistant-gitlab-auth](https://www.npmjs.com/package/myassistant-gitlab-auth).

##### Konfiguracja

Customize through `myassistant.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem myassistant:
...
Aby używać Google Vertex AI z myassistant:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania myassistant.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą myassistant, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [myassistant-helicone-session](https://github.com/H2Shami/myassistant-helicone-session), aby automatycznie rejestrować każdą konwersację myassistant jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja myassistant będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować myassistant tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować myassistant tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować myassistant tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem myassistant. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/myassistant).
:::
...
Aby korzystać z Ollama Cloud z myassistant:
...
3. Skopiuj klucz API do użycia w myassistant.
...
6. **Ważne**: Przed użyciem modeli chmurowych w myassistant musisz pobrać informacje o modelu lokalnie:
...
MyAssistant Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół myassistant. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>MyAssistant Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **MyAssistant Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API myassistant.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji myassistant.
...
5. Możesz je także dostosować za pomocą konfiguracji myassistant. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z myassistant:
...
Możesz także dostosować modele za pomocą konfiguracji myassistant. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z myassistant. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `myassistant.json` w katalogu projektu:
...
Pola `limit` pozwalają myassistant zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `myassistant auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację myassistant i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji myassistant.
