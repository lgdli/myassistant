...
MyAssistant Zen to lista modeli dostarczonych przez zespół opencode, które zostały
...
1. Uruchom polecenie `/connect` w TUI, wybierz `MyAssistant Zen` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
MyAssistant Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół opencode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z opencode.

1. Uruchom polecenie `/connect` w TUI, wybierz `MyAssistant Go` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
MyAssistant Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół opencode. [Dowiedz się więcej](/docs/zen).
