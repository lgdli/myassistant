...
Umiejętności agenta pozwalają myassistant odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
myassistant przeszukuje te lokalizacje:

- Project config: `.myassistant/skills/<name>/SKILL.md`
- Global config: `~/.config/myassistant/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu myassistant przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.myassistant/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/myassistant/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.myassistant/skills/git-release/SKILL.md` w ten sposób:
...
myassistant wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `myassistant.json`:
...
**Dla agentów wbudowanych** (w `myassistant.json`):
