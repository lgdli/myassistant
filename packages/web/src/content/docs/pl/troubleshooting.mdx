...
Aby debugować problemy z myassistant, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\myassistant\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `myassistant --log-level DEBUG`.

---

## Przechowywanie danych

myassistant przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/myassistant/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\myassistant`
...
myassistant Desktop uruchamia w tle lokalny serwer myassistant (sidecar `myassistant-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `MyAssistant` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/myassistant/myassistant.jsonc` (lub `~/.config/myassistant/myassistant.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/myassistant/myassistant.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\myassistant\myassistant.jsonc`
...
myassistant może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/myassistant/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\myassistant\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby myassistant mógł ją odbudować.

1. Całkowicie zamknij myassistant Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/myassistant`
- **Linux**: usuń `~/.cache/myassistant` (lub uruchom `rm -rf ~/.cache/myassistant`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\myassistant`

3. Uruchom ponownie myassistant Desktop.

---

### Napraw problemy z połączeniem z serwerem

myassistant Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `myassistant.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows myassistant Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami myassistant.

---

### Brak powiadomień

myassistant Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla myassistant w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie myassistant Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji myassistant Desktop):

- `myassistant.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `myassistant.global.dat` i `myassistant.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z myassistant:
...
3. Upewnij się, że masz najnowszą wersję z `myassistant upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `myassistant models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\myassistant`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. myassistant dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\myassistant`

2. Uruchom ponownie kod myassistant, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to myassistant do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
myassistant wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
