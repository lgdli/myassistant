---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć myassistant.
---

Wtyczki umożliwiające wyłączenie myassistant poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania myassistant.
...
- `.myassistant/plugins/` - Wtyczki na poziomie projektu
- `~/.config/myassistant/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/myassistant/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/myassistant/myassistant.json`)
2. Konfiguracja projektu (`myassistant.json`)
3. Globalny katalog wtyczek (`~/.config/myassistant/plugins/`)
4. Katalog wtyczek projektu (`.myassistant/plugins/`)
...
myassistant uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z myassistant do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do myassistant.
...
:::note
Jeśli korzystasz z aplikacji komputerowej myassistant, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj myassistant czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do myassistant:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod myassistant. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://myassistant.ai/docs/sdk).
