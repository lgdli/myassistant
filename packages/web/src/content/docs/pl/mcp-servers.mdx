...
Możesz dodać dodatkowe informacje do myassistant za pomocą _Model Context Protocol_, w skrócie MCP. myassistant obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [myassistant Config](https://myassistant.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/myassistant`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
myassistant automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, myassistant:
...
Jeśli serwer wymaga uwierzytelnienia, myassistant poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `myassistant mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji myassistant bezpieczne przechowa tokeny w `~/.local/share/myassistant/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w myassistant, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie myassistant, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć myassistant z kontem Sentry.
