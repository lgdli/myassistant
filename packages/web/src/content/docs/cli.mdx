---
title: CLI
description: MyAssistant CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The MyAssistant CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
myassistant
```

But it also accepts commands as documented on this page. This allows you to interact with MyAssistant programmatically.
...
Start the MyAssistant terminal user interface.
...
The MyAssistant CLI also has the following commands.

---

### agent

Manage agents for MyAssistant.
...
Attach a terminal to an already running MyAssistant backend server started via `serve` or `web` commands.

```bash
myassistant attach [url]
```

This allows using the TUI with a remote MyAssistant backend. For example:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `myassistant`) |
...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.myassistant/agent` based on the prompt)                                                                                                                         |
...
MyAssistant is powered by the provider list at [Models.dev](https://models.dev), so you can use `myassistant auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/myassistant/auth.json`.

```bash
myassistant auth login
```

When MyAssistant starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in MyAssistant.
...
Run myassistant in non-interactive mode by passing a prompt directly.
...
You can also attach to a running `myassistant serve` instance to avoid MCP server cold boot times on every run:
...
| <nobr><code>{"--attach"}</code></nobr>                       |       | Attach to a running myassistant server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr>                     | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr>                     | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `myassistant`) |
...
Start a headless MyAssistant server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
myassistant serve
```

This starts an HTTP server that provides API access to myassistant functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `myassistant`).
...
Delete an MyAssistant session.

```bash
myassistant session delete <sessionID>
```
...
Show token usage and cost statistics for your MyAssistant sessions.
...
Import session data from a JSON file or MyAssistant share URL.

```bash
myassistant import <file>
```

You can import from a local file or an MyAssistant share URL.
...
Start a headless MyAssistant server with a web interface.

```bash
myassistant web
```

This starts an HTTP server and opens a web browser to access MyAssistant through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `myassistant`).
...
```bash
myassistant plugin <module>
```

Or use the alias.

```bash
myassistant plug <module>
```
...
Fetch and checkout a GitHub PR branch, then run MyAssistant.

```bash
myassistant pr <number>
```
...
Uninstall MyAssistant and remove all related files.
...
Updates myassistant to the latest version or a specific version.
...
The myassistant CLI takes the following global flags.
...
MyAssistant can be configured using environment variables.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `myassistant`) |
