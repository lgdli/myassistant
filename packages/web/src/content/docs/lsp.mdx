---
title: LSP Servers
description: MyAssistant integrates with your LSP servers.
---

MyAssistant can integrate with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

MyAssistant comes with several built-in LSP servers for popular languages:
...
When LSP is enabled and myassistant opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your myassistant config.
...
```json title="myassistant.json" {5-8}
{
  "$schema": "https://myassistant.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="myassistant.json" {5-13}
{
  "$schema": "https://myassistant.ai/config.json",
  "lsp": {
    "custom-lsp": {
