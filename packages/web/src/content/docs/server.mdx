---
title: Server
description: Interact with myassistant server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `myassistant serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an myassistant client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `myassistant.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `myassistant`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `myassistant serve` and `myassistant web`.
...
When you run `myassistant` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the myassistant server to interact with myassistant programmatically.
:::

This architecture lets myassistant support multiple clients and allows you to interact with myassistant programmatically.

You can run `myassistant serve` to start a standalone server. If you have the
myassistant TUI running, `myassistant serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the MyAssistant [IDE](/docs/ide) plugins.
...
The myassistant server exposes the following APIs.
