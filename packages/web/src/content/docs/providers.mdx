---
title: Providers
description: Using any LLM provider in MyAssistant.
---

import config from "../../../config.mjs"
export const console = config.console

MyAssistant uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your MyAssistant config.
...
You can customize the providers through the `provider` section in your MyAssistant
config.
...
MyAssistant Zen is a list of models provided by the MyAssistant team that have been
tested and verified to work well with MyAssistant. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with MyAssistant Zen.
:::

1. Run the `/connect` command in the TUI, select `MyAssistant Zen`, and head to [myassistant.ai/auth](https://myassistant.ai/zen).
...
It works like any other provider in MyAssistant and is completely optional to use.

---

## MyAssistant Go

MyAssistant Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the MyAssistant team that have been
tested and verified to work well with MyAssistant.

1. Run the `/connect` command in the TUI, select `MyAssistant Go`, and head to [myassistant.ai/auth](https://myassistant.ai/zen).
...
To use Amazon Bedrock with MyAssistant:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running myassistant:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `myassistant.json`:
...
MyAssistant. Anthropic explicitly prohibits this.

Previous versions of MyAssistant came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in MyAssistant with zero setup:
...
You can configure myassistant to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for myassistant to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your myassistant config.
...
GitLab Duo support in MyAssistant is experimental. Features, configuration, and
behavior may change in future releases.
:::

MyAssistant integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
MyAssistant uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock MyAssistant
to only use your own GitLab-hosted instance, add the following to your
`myassistant.json` file. It is also recommended to disable session sharing.
...
More documentation on [myassistant-gitlab-auth](https://www.npmjs.com/package/myassistant-gitlab-auth) homepage.
...
When a `duo-workflow-*` model is selected, MyAssistant will:
...
4. Route tool execution requests through MyAssistant's permission-gated tool system
...
To use your GitHub Copilot subscription with myassistant:
...
To use Google Vertex AI with MyAssistant:
...
2. Set the required environment variables:
   ...
   Set them while running myassistant.
...
In the event you see a feature or model from Helicone that isn't configured automatically through myassistant, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [myassistant-helicone-session](https://github.com/H2Shami/myassistant-helicone-session) plugin to automatically log each MyAssistant conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each MyAssistant conversation listed as a separate session.
...
You can configure myassistant to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure myassistant to use local models through LM Studio.
...
You can configure myassistant to use local models through Ollama.

:::tip
Ollama can automatically configure itself for MyAssistant. See the [Ollama integration docs](https://docs.ollama.com/integrations/myassistant) for details.
:::
...
To use Ollama Cloud with MyAssistant:
...
3. Copy the API key for use in MyAssistant.
...
6. **Important**: Before using cloud models in MyAssistant, you must pull the model information locally:
...
MyAssistant Zen is a list of tested and verified models provided by the MyAssistant team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>MyAssistant Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **MyAssistant Zen**.

   ```txt
   /connect
   ```

3. Enter your MyAssistant API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your myassistant config.
...
5. You can also customize them through your myassistant config. Here's an example of specifying a provider
...
You can also customize models through your myassistant config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with myassistant. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `myassistant.json` file in your project directory:
...
The `limit` fields allow MyAssistant to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `myassistant auth list` to see if the credentials
...
2. For custom providers, check the myassistant config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your myassistant config.
