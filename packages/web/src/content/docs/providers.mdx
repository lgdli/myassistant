...
1. Run the `/connect` command in the TUI, select `MyAssistant Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
1. Run the `/connect` command in the TUI, select `MyAssistant Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each MyAssistant conversation as a session in Helicone.
...
:::tip
Ollama can automatically configure itself for MyAssistant. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
