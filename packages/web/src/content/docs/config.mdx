---
title: Config
description: Using the MyAssistant JSON config.
---

You can configure MyAssistant using a JSON config file.

---

## Format

MyAssistant supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="myassistant.jsonc"
{
...
1. **Remote config** (from `.well-known/myassistant`) - organizational defaults
2. **Global config** (`~/.config/myassistant/myassistant.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`myassistant.json` in project) - project-specific settings
5. **`.myassistant` directories** - agents, commands, plugins
6. **Inline config** (`OPENCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/myassistant/` on macOS) - admin-controlled
...
:::note
The `.myassistant` and `~/.config/myassistant` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/myassistant` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/myassistant"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global MyAssistant config in `~/.config/myassistant/myassistant.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/myassistant/tui.json`.
...
Add `myassistant.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When MyAssistant starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
modes, and plugins just like the standard `.myassistant` directory, and should
...
The custom directory is loaded after the global config and `.myassistant` directories, so it **can override** their settings.
...
Drop an `myassistant.json` or `myassistant.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/myassistant/` |
| Linux    | `/etc/myassistant/`                         |
| Windows  | `%ProgramData%\myassistant`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, MyAssistant reads managed preferences from the `ai.myassistant.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

MyAssistant checks these paths:

1. `/Library/Managed Preferences/<user>/ai.myassistant.managed.plist`
2. `/Library/Managed Preferences/ai.myassistant.managed.plist`

The plist keys map directly to `myassistant.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.myassistant.managed` PayloadType. The MyAssistant config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.myassistant.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.myassistant.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.myassistant</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`myassistant.ai/config.json`**](https://myassistant.ai/config.json).

TUI config uses [**`myassistant.ai/tui.json`**](https://myassistant.ai/tui.json).
...
Legacy `theme`, `keybinds`, and `tui` keys in `myassistant.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `myassistant serve` and `myassistant web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your MyAssistant server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `myassistant.local`. Useful for running multiple instances on the same network.
...
If not specified, MyAssistant will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your MyAssistant config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, MyAssistant tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
You can also define agents using markdown files in `~/.config/myassistant/agents/` or `.myassistant/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, MyAssistant will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`myassistant run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/myassistant/commands/` or `.myassistant/commands/`. [Learn more here](/docs/commands).
...
MyAssistant uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
MyAssistant will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, myassistant **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend MyAssistant with custom tools, hooks, and integrations.

Place plugin files in `.myassistant/plugins/` or `~/.config/myassistant/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="myassistant.json"
{
  "$schema": "https://myassistant.ai/config.json",
  "plugin": ["myassistant-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict MyAssistant to only use specific providers rather than disabling them one by one.
