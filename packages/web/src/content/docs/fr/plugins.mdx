---
title: Plugins
description: Écrivez vos propres plugins pour étendre MyAssistant.
---

Les plugins vous permettent d'étendre MyAssistant en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de MyAssistant.
...
- `.myassistant/plugins/` - Plugins au niveau du projet
- `~/.config/myassistant/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/myassistant/node_modules/`.
...
1. Configuration globale (`~/.config/myassistant/myassistant.json`)
2. Configuration du projet (`myassistant.json`)
3. Répertoire global des plugins (`~/.config/myassistant/plugins/`)
4. Répertoire des plugins du projet (`.myassistant/plugins/`)
...
MyAssistant exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK myassistant pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre myassistant.
...
:::note
Si vous utilisez l'application de bureau MyAssistant, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez myassistant de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à myassistant :
...
L'assistant `tool` crée un outil personnalisé que myassistant peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour myassistant aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://myassistant.ai/docs/sdk) pour plus de détails.
