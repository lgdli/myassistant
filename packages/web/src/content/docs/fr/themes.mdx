...
Avec MyAssistant, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, MyAssistant utilise notre propre thème `myassistant`.
...
MyAssistant est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que MyAssistant corresponde à l'apparence de leur terminal
...
MyAssistant prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/myassistant/themes/*.json` ou `$XDG_CONFIG_HOME/myassistant/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.myassistant/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.myassistant/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/myassistant/themes
vim ~/.config/myassistant/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .myassistant/themes
vim .myassistant/themes/my-theme.json
```
...
{
  "$schema": "https://myassistant.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
