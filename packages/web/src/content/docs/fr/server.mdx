---
title: Serveur
description: Interagissez avec le serveur myassistant via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `myassistant serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client myassistant peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `myassistant.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `myassistant`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `myassistant serve` et à `myassistant web`.
...
Lorsque vous exécutez `myassistant`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur myassistant pour interagir avec myassistant par programmation.
:::

Cette architecture permet à myassistant de prendre en charge plusieurs clients et vous permet d'interagir avec myassistant par programme.

Vous pouvez exécuter `myassistant serve` pour démarrer un serveur autonome. Si vous avez le
myassistant TUI en cours d'exécution, `myassistant serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins MyAssistant [IDE](/docs/ide).
...
Le serveur myassistant expose les API suivantes.
