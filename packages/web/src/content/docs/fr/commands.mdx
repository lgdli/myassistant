...
Créez `.myassistant/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration MyAssistant ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre MyAssistant [config](/docs/config) :
...
- Par projet : `.myassistant/commands/`
...
myassistant comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
