---
title: Formateurs
description: MyAssistant utilise des formateurs spécifiques au langage.
---

MyAssistant formate automatiquement les fichiers après leur écriture ou leur modification à l'aide de formateurs spécifiques au langage. Cela garantit que le code généré suit les styles de code de votre projet.

---

## Formateurs intégrés

MyAssistant est livré avec plusieurs formateurs intégrés pour les langages et frameworks populaires. Vous trouverez ci-dessous une liste des formateurs, des extensions de fichiers prises en charge et des commandes ou options de configuration dont il a besoin.
...
Ainsi, si votre projet a `prettier` dans votre `package.json`, MyAssistant l'utilisera automatiquement.

---

## Comment ça marche

Lorsque MyAssistant écrit ou modifie un fichier, il :
...
Vous pouvez personnaliser les formateurs via la section `formatter` de votre configuration MyAssistant.
...
```json title="myassistant.json" {4-14}
{
  "$schema": "https://myassistant.ai/config.json",
  "formatter": {
    "prettier": {
