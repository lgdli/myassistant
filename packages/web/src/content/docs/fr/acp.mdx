---
title: Support ACP
description: Utilisez MyAssistant dans n’importe quel éditeur compatible ACP.
---

MyAssistant prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser MyAssistant via ACP, configurez votre éditeur pour exécuter la commande `myassistant acp`.

La commande démarre MyAssistant en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "MyAssistant": {
      "command": "/absolute/path/bin/myassistant",
      "args": ["acp"]
    }
  }
}
...
Pour l'ouvrir, utilisez le nouvel agent « MyAssistant » dans le sélecteur d'agent AI Chat.
...
Pour utiliser MyAssistant comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser MyAssistant comme agent ACP pour le chat.
...
MyAssistant fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration MyAssistant
