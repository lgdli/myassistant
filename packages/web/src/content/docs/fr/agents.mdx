...
Il existe deux types d'agents dans MyAssistant ; agents primaires et sous-agents.
...
MyAssistant est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
MyAssistant est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

MyAssistant est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `myassistant.json` :
...
- Global : `~/.config/myassistant/agents/`
- Par projet : `.myassistant/agents/`

```markdown title="~/.config/myassistant/agents/review.md"
---
...
Si aucune température n'est spécifiée, MyAssistant utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale MyAssistant et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration MyAssistant utilise le format `provider/model-id`. Par exemple, si vous utilisez [MyAssistant Zen](/docs/zen), vous utiliserez `myassistant/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="myassistant.json" {3-6,9-12}
{
  "$schema": "https://myassistant.ai/config.json",
  "tools": {
    "write": true,
...
```json title="myassistant.json" {3-5,8-10}
{
  "$schema": "https://myassistant.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="myassistant.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Exécutez `myassistant models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/myassistant).
:::
...
```markdown title="~/.config/myassistant/agents/docs-writer.md"
---
...
```markdown title="~/.config/myassistant/agents/security-auditor.md"
---
