---
title: CLI
description: MyAssistant CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der MyAssistant CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
myassistant
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit MyAssistant interagieren.

```bash
myassistant run "Explain how closures work in JavaScript"
```
...
Starten Sie die Terminalbenutzeroberfläche MyAssistant.
...
Der MyAssistant CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für MyAssistant verwalten.

```bash
myassistant agent [command]
```
...
Schließen Sie ein Terminal an einen bereits laufenden MyAssistant-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
myassistant attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend MyAssistant. Zum Beispiel:

```bash
# Start the backend server for web/mobile access
myassistant web --port 4096 --hostname 0.0.0.0

# In another terminal, attach the TUI to the running backend
myassistant attach http://10.20.30.40:4096
```
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `myassistant`) |
...
MyAssistant wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `myassistant auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/myassistant/auth.json` gespeichert.

```bash
myassistant auth login
```

Wenn MyAssistant startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
```bash
myassistant mcp auth list
```

Oder nutzen Sie die Kurzversion.

```bash
myassistant mcp auth ls
```
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in MyAssistant sehen möchten.

```bash
myassistant models --refresh
```
...
Führen Sie MyAssistant im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.

```bash
myassistant run [message..]
```
...
```bash "myassistant run"
myassistant run Explain the use of context in Go
```

Sie können auch eine Verbindung zu einer laufenden `myassistant serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
myassistant run --attach http://localhost:4096 "Explain async/await in JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden MyAssistant-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `myassistant`)                  |
...
Starten Sie einen Headless-MyAssistant-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
myassistant serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf MyAssistant-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `myassistant`).
...
MyAssistant-Sitzungen verwalten.

```bash
myassistant session [command]
```
...
Listen Sie alle MyAssistant-Sitzungen auf.

```bash
myassistant session list
```
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre MyAssistant-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer MyAssistant-Freigabe-URL.

```bash
myassistant import <file>
```

Sie können aus einer lokalen Datei oder einer MyAssistant-Freigabe-URL importieren.

```bash
myassistant import session.json
myassistant import https://opncd.ai/s/abc123
```
...
Starten Sie einen Headless-MyAssistant-Server mit einer Weboberfläche.

```bash
myassistant web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf MyAssistant zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `myassistant`).
...
Deinstallieren Sie MyAssistant und entfernen Sie alle zugehörigen Dateien.

```bash
myassistant uninstall
```
...
Aktualisiert MyAssistant auf die neueste Version oder eine bestimmte Version.

```bash
myassistant upgrade [target]
```
...
Der MyAssistant CLI akzeptiert die folgenden globalen Flags.
...
MyAssistant kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `myassistant`) |
