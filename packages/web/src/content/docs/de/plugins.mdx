---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um MyAssistant zu erweitern.
---

Mit Plugins können Sie MyAssistant erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von MyAssistant zu ändern.
...
- `.myassistant/plugins/` – Plugins auf Projektebene
- `~/.config/myassistant/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/myassistant/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/myassistant/myassistant.json`)
2. Projektkonfiguration (`myassistant.json`)
3. Globales Plugin-Verzeichnis (`~/.config/myassistant/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.myassistant/plugins/`)
...
MyAssistant führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein MyAssistant-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie MyAssistant erweitern können.
...
:::note
Wenn Sie die MyAssistant-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass MyAssistant `.env`-Dateien liest:
...
Plugins können MyAssistant auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das MyAssistant aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für MyAssistant zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://myassistant.ai/docs/sdk).
