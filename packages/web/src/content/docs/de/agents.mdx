...
Es gibt zwei Arten von Agenten in MyAssistant; Primäragenten und Subagenten.
...
MyAssistant verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `myassistant.json`-Konfigurationsdatei:
...
- Pro Projekt: `.myassistant/agents/`
...
Wenn keine Temperatur angegeben ist, verwendet MyAssistant modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale MyAssistant-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer MyAssistant-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [MyAssistant Zen](/docs/zen) verwenden, würden Sie `myassistant/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `myassistant models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/myassistant).
:::
