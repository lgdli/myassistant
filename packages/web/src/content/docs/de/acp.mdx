...
description: Verwenden Sie MyAssistant in jedem ACP-kompatiblen Editor.
---

MyAssistant unterstützt [Agent Client Protocol](https://agentclientprotocol.com) oder (ACP), sodass Sie es direkt in kompatiblen Editoren und IDEs verwenden können.
...
Um MyAssistant über ACP zu verwenden, konfigurieren Sie Ihren Editor für die Ausführung des Befehls `myassistant acp`.

Der Befehl startet MyAssistant als ACP-kompatiblen Unterprozess, der über stdio über JSON-RPC mit Ihrem Editor kommuniziert.
...
Um es zu öffnen, verwenden Sie den neuen Agenten „MyAssistant“ in der AI Chat Agent Selector.
...
Um MyAssistant als ACP-Agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim) zu verwenden, fügen Sie Folgendes zu Ihrer Neovim-Konfiguration hinzu:
...
Diese Konfiguration richtet CodeCompanion so ein, dass MyAssistant als ACP-Agent für den Chat verwendet wird.
...
MyAssistant funktioniert über ACP genauso wie im Terminal. Alle Funktionen werden unterstützt:
...
- MCP-Server, die in Ihrer MyAssistant-Konfiguration konfiguriert sind
