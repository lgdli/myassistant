...
description: Erstellen Sie Tools, die der LLM in MyAssistant aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von MyAssistant wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.myassistant/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/myassistant/tools/` platzieren.
