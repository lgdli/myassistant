...
Agent Skills erlauben MyAssistant, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
MyAssistant sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft MyAssistant vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.myassistant/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/myassistant/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.myassistant/skills/git-release/SKILL.md` zum Beispiel so:
...
MyAssistant listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `myassistant.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `myassistant.json`):
