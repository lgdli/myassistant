---
title: Server
description: Nutze den myassistant-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `myassistant serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein myassistant-Client nutzen kann.
...
Der Benutzername ist standardmaessig `myassistant`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `myassistant serve` und `myassistant web`.
...
Wenn du `myassistant` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den myassistant-Server, um myassistant programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `myassistant serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `myassistant serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die MyAssistant-[IDE](/docs/ide)-Plugins.
...
Der myassistant-Server stellt folgende APIs bereit.
