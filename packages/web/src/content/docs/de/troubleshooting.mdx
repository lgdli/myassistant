...
Wenn MyAssistant Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\myassistant\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `myassistant --log-level DEBUG`.

---

## Speicher

myassistant speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/myassistant/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\myassistant`
...
MyAssistant Desktop startet im Hintergrund einen lokalen MyAssistant-Server (`myassistant-cli`-Sidecar).
...
- Nur macOS: `MyAssistant`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/myassistant/myassistant.jsonc` (oder `~/.config/myassistant/myassistant.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/myassistant/myassistant.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\myassistant\myassistant.jsonc`
...
MyAssistant kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/myassistant/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\myassistant\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit MyAssistant ihn neu aufbauen kann.

1. Beende MyAssistant Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/myassistant`
- **Linux**: loesche `~/.cache/myassistant` (oder fuehre aus: `rm -rf ~/.cache/myassistant`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\myassistant`

3. Starte MyAssistant Desktop neu.

---

### Server-Verbindungsprobleme beheben

MyAssistant Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `myassistant.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt MyAssistant Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit MyAssistant-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

MyAssistant Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer MyAssistant in den OS-Einstellungen aktiviert sind, und
...
1. Beende MyAssistant Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von MyAssistant Desktop):

- `myassistant.settings.dat` (desktop default server URL)
- `myassistant.global.dat` und `myassistant.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit MyAssistant hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `myassistant models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\myassistant`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. myassistant installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\myassistant`

2. Starte myassistant neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt myassistant, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
myassistant erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
