---
title: Regeln
description: Lege eigene Anweisungen fuer myassistant fest.
---

Du kannst myassistant ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in myassistant den Befehl `/init` aus.
...
So kann myassistant sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht myassistant sie zu erweitern.
...
myassistant kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/myassistant/AGENTS.md` speichern.
Diese gelten in allen myassistant-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt MyAssistant dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/myassistant/AGENTS.md` existiert)
...
Beim Start sucht myassistant in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/myassistant/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `myassistant.json` oder in `~/.config/myassistant/myassistant.json` angeben.
...
myassistant parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit myassistant.json

Empfohlen ist das `instructions`-Feld in der `myassistant.json`:
...
Du kannst myassistant explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `myassistant.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
