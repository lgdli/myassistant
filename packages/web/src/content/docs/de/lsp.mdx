---
title: LSP Server
description: MyAssistant lässt sich in Ihren LSP-Server integrieren.
---

MyAssistant lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

MyAssistant verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
Wenn MyAssistant eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer MyAssistant-Konfiguration anpassen.
...
```json title="myassistant.json" {5-7}
{
  "$schema": "https://myassistant.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="myassistant.json" {5-9}
{
  "$schema": "https://myassistant.ai/config.json",
  "lsp": {
    "typescript": {
