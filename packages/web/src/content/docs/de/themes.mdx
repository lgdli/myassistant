...
Mit MyAssistant kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet MyAssistant das `myassistant`-Theme.
...
MyAssistant liefert mehrere eingebaute Themes mit.
...
- MyAssistant optisch ans Terminal anpassen wollen
...
MyAssistant unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/myassistant/themes/*.json` oder `$XDG_CONFIG_HOME/myassistant/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.myassistant/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.myassistant/themes/*.json`
