---
title: LSPサーバー
description: MyAssistant は LSP サーバーと統合します。
---

MyAssistant は言語サーバープロトコル (LSP) と統合して、LLM がコードベースと対話できるようにします。診断を使用して LLM にフィードバックを提供します。

---

## 組み込み

MyAssistant には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
MyAssistant がファイルを開くと、次のようになります。
...
MyAssistant 設定の `lsp` セクションを通じて LSP サーバーをカスタマイズできます。
