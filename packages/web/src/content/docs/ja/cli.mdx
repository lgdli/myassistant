---
title: CLI
description: MyAssistant CLI のオプションとコマンド。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MyAssistant CLI は、引数なしで実行すると、デフォルトで [TUI](/docs/tui) を開始します。

```bash
myassistant
```

ただし、このページに記載されているようにコマンドも受け入れます。これにより、MyAssistant をプログラム的に操作できるようになります。
...
MyAssistant ターミナルユーザーインターフェイスを開始します。
...
MyAssistant CLI には次のコマンドもあります。

---

### agent

MyAssistant のエージェントを管理します。
...
`serve` または `web` コマンドを使用して起動された、すでに実行中の MyAssistant バックエンドサーバーにターミナルを接続します。

```bash
myassistant attach [url]
```

これにより、リモート MyAssistant バックエンドで TUI を使用できるようになります。例えば：
...
| <nobr><code>{"--username"}</code></nobr> | `-u`     | Basic 認証ユーザー名（デフォルトは `OPENCODE_SERVER_USERNAME` または `myassistant`） |
...
MyAssistant は [Models.dev](https://models.dev) のプロバイダーリストを利用しているため、`myassistant auth login` を使用して、使用したいプロバイダーの API キーを構成できます。これは `~/.local/share/myassistant/auth.json` に保存されます。

```bash
myassistant auth login
```

MyAssistant が起動すると、認証情報ファイルからプロバイダーがロードされます。また、環境またはプロジェクト内の `.env` ファイルで定義されたキーがあるかどうかも確認します。
...
`--refresh` フラグを使用して、キャッシュされたモデルリストを更新します。これは、新しいモデルがプロバイダーに追加され、それを MyAssistant で確認したい場合に便利です。
...
プロンプトを直接渡して、非対話モードで myassistant を実行します。
...
実行中の `myassistant serve` インスタンスにアタッチして、実行ごとの MCP サーバーのコールドブート時間を回避することもできます。
...
| <nobr><code>{"--attach"}</code></nobr>   |          | 実行中の myassistant サーバー (http://localhost:4096 など) に接続します。                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`     | Basic 認証パスワード（デフォルトは `OPENCODE_SERVER_PASSWORD`）                           |
| <nobr><code>{"--username"}</code></nobr> | `-u`     | Basic 認証ユーザー名（デフォルトは `OPENCODE_SERVER_USERNAME` または `myassistant`）         |
...
API アクセスのためにヘッドレス MyAssistant サーバーを起動します。完全な HTTP インターフェイスについては、[サーバーのドキュメント](/docs/server) を確認してください。

```bash
myassistant serve
```

これにより、TUI インターフェイスを使用せずに myassistant 機能への API アクセスを提供する HTTP サーバーが起動します。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `myassistant`)。
...
MyAssistant セッションのトークンの使用状況とコストの統計を表示します。
...
JSON ファイルまたは MyAssistant 共有 URL からセッションデータをインポートします。

```bash
myassistant import <file>
```

ローカルファイルまたは MyAssistant 共有 URL からインポートできます。
...
Web インターフェイスを使用してヘッドレス MyAssistant サーバーを起動します。

```bash
myassistant web
```

これにより、HTTP サーバーが起動し、Web ブラウザが開き、Web インターフェイスを通じて MyAssistant にアクセスします。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `myassistant`)。
...
MyAssistant をアンインストールし、関連ファイルをすべて削除します。
...
myassistant を最新バージョンまたは特定のバージョンに更新します。
...
myassistant CLI は次のグローバルフラグを受け取ります。
...
| `OPENCODE_SERVER_USERNAME`            | 文字列   | 基本認証ユーザー名 (デフォルト `myassistant`) をオーバーライドします。 |
