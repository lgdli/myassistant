---
title: サーバー
description: HTTP 経由で MyAssistant サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`myassistant serve` コマンドは、MyAssistant クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `myassistant.local` |
...
HTTP 基本認証でサーバーを保護するには、`OPENCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `myassistant` になるか、`OPENCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`myassistant serve` と `myassistant web` の両方に当てはまります。
...
`myassistant` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
MyAssistant サーバーを使用して、プログラムで MyAssistant と対話します。
:::

`myassistant serve` を実行してスタンドアロンサーバーを起動できます。すでに MyAssistant TUI を実行している場合でも、`myassistant serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、MyAssistant [IDE](/docs/ide) プラグインによって使用されます。
...
MyAssistant サーバーは次の API を公開します。
