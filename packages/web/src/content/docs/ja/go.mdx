...
MyAssistant Goは低価格のサブスクリプションで、**初月は5ドル**、その後は**月額10ドル**で、人気のオープンなコーディングモデルに安定してアクセスできます。

:::note
MyAssistant Goは現在ベータ版です。
:::

GoはMyAssistantの他のプロバイダーと同様に機能します。MyAssistant GoをサブスクライブしてAPIキーを取得します。これは**完全に任意**であり、MyAssistantを使用するために必須ではありません。
...
:::tip
MyAssistantでうまく動作する一部のモデルとプロバイダーをテストしました。
:::
...
MyAssistant Goを使用すると、これらのモデルに**初月は5ドル**、その後は**月額10ドル**でアクセスできます。

---

## 仕組み

MyAssistant Goは、MyAssistantの他のプロバイダーと同様に機能します。

1. **<a href={console}>MyAssistant Zen</a>**にサインインし、GoをサブスクライブしてAPIキーをコピーします。
2. TUIで`/connect`コマンドを実行し、`MyAssistant Go`を選択して、APIキーを貼り付けます。
3. TUIで`/models`を実行すると、Goを通じて利用可能なモデルのリストが表示されます。

:::note
MyAssistant Goをサブスクライブできるのは、1つのワークスペースにつき1メンバーのみです。
:::
...
MyAssistant設定の[model id](/docs/config/#models)は、`myassistant-go/<model-id>`という形式を使用します。たとえば、Kimi K2.6の場合は、設定で`myassistant-go/kimi-k2.6`を使用します。
...
4. MyAssistantで他のプロバイダーも使用できるようにすることで、**ベンダーロックインをなくす**。
