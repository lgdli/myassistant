---
title: CLI
description: MyAssistant CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MyAssistant CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
myassistant
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med MyAssistant programmatisk.
...
Start MyAssistant-terminalbrukergrensesnittet.
...
Administrer agenter for MyAssistant.
...
Koble en terminal til en allerede kjørende MyAssistant-backend-server startet via `serve` eller `web` kommandoer.

```bash
myassistant attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern MyAssistant-backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `myassistant`) |
...
MyAssistant drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `myassistant auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/myassistant/auth.json`.

```bash
myassistant auth login
```

Når MyAssistant starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i MyAssistant.
...
Kjør MyAssistant i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `myassistant serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende MyAssistant-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `myassistant`) |
...
Start en headless MyAssistant-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
myassistant serve
```

Dette starter en HTTP-server som gir API tilgang til MyAssistant-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `myassistant`).
...
List opp alle MyAssistant-økter.
...
Vis tokenbruk og kostnadsstatistikk for MyAssistant-øktene dine.
...
Importer øktdata fra en JSON-fil eller en MyAssistant share-URL.
...
Start en headless MyAssistant-server med et webgrensesnitt.

```bash
myassistant web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til MyAssistant via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `myassistant`).
...
Avinstaller MyAssistant og fjern alle relaterte filer.
...
Oppdaterer MyAssistant til siste versjon eller en spesifikk versjon.
...
MyAssistant CLI bruker følgende globale flagg.
...
MyAssistant kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `myassistant`) |
