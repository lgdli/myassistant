...
Du kan legge til eksterne verktøy i MyAssistant ved å bruke _Model Context Protocol_ eller MCP. MyAssistant støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [MyAssistant Config](https://myassistant.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="myassistant.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/myassistant`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="myassistant.jsonc" {15}
{
  "$schema": "https://myassistant.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
MyAssistant håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil MyAssistant:
...
Hvis serveren krever autentisering, vil MyAssistant be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `myassistant mcp auth <server-name>`.
...
```json title="myassistant.json" {7-11}
{
  "$schema": "https://myassistant.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
myassistant mcp auth my-oauth-server
```
...
```bash
myassistant mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil MyAssistant lagre tokens sikkert i `~/.local/share/myassistant/mcp-auth.json`.
...
myassistant mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i MyAssistant, sammen med innebygde verktøy. Så du kan administrere dem gjennom MyAssistant-konfigurasjonen som alle andre verktøy.
...
```json title="myassistant.json" {14}
{
  "$schema": "https://myassistant.ai/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="myassistant.json" {11, 14-18}
{
  "$schema": "https://myassistant.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="myassistant.json" {4-8}
{
  "$schema": "https://myassistant.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
myassistant mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble MyAssistant til Sentry-kontoen din.
...
```json title="myassistant.json" {4-7}
{
  "$schema": "https://myassistant.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="myassistant.json" {7-9}
{
  "$schema": "https://myassistant.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver MyAssistant-samtale oppført som en separat økt.
