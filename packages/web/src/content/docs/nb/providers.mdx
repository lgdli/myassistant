---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i MyAssistant.
---

import config from "../../../../config.mjs"
export const console = config.console

MyAssistant bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i MyAssistant-konfigurasjonen.
...
i `~/.local/share/myassistant/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i MyAssistant
konfig.
...
MyAssistant Zen er en liste over modeller levert av MyAssistant-teamet som har vært
testet og verifisert for å fungere godt med MyAssistant. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med MyAssistant Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `MyAssistant Zen` og gå til [myassistant.ai/auth](https://myassistant.ai/zen).
...
Det fungerer som alle andre leverandører i MyAssistant og er helt valgfritt å bruke.

---

## MyAssistant Go

MyAssistant Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av MyAssistant-teamet som har vært
testet og verifisert for å fungere godt med MyAssistant.

1. Kjør kommandoen `/connect` i TUI, velg `MyAssistant Go`, og gå til [myassistant.ai/auth](https://myassistant.ai/zen).
...
Slik bruker du Amazon Bedrock med MyAssistant:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører MyAssistant:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `myassistant.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i myassistant støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere myassistant til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at MyAssistant skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom MyAssistant-konfigurasjonen.
...
for å lagre token i MyAssistant auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
MyAssistant bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse MyAssistant
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`myassistant.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [myassistant-gitlab-auth](https://www.npmjs.com/package/myassistant-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `myassistant.json`:
...
Slik bruker du GitHub Copilot-abonnementet med MyAssistant:
...
Slik bruker du Google Vertex AI med MyAssistant:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører MyAssistant.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom MyAssistant, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [myassistant-helicone-session](https://github.com/H2Shami/myassistant-helicone-session) plugin for automatisk å logge hver MyAssistant-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver MyAssistant-samtale oppført som en separat økt.
...
Du kan konfigurere MyAssistant for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere MyAssistant for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere MyAssistant for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for MyAssistant. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/myassistant) for detaljer.
:::
...
Slik bruker du Ollama Cloud med MyAssistant:
...
3. Kopier API-nøkkelen for bruk i MyAssistant.
...
6. **Viktig**: Før du bruker skymodeller i MyAssistant, må du hente modellinformasjonen lokalt:
...
MyAssistant Zen er en liste over testede og verifiserte modeller levert av MyAssistant-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>MyAssistant Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **MyAssistant Zen**.

   ```txt
   /connect
   ```

3. Skriv inn MyAssistant API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom MyAssistant-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom MyAssistant-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med MyAssistant:
...
Du kan også tilpasse modeller gjennom MyAssistant-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med MyAssistant. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `myassistant.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar MyAssistant forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `myassistant auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk MyAssistant-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i MyAssistant-konfigurasjonen.
