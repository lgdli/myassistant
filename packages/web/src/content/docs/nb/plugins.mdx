...
description: Skriv dine egne programtillegg for å utvide MyAssistant.
---

Plugins lar deg utvide MyAssistant ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til MyAssistant.
...
- `.myassistant/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/myassistant/node_modules/`.
...
1. Global konfigurasjon (`~/.config/myassistant/myassistant.json`)
2. Prosjektkonfigurasjon (`myassistant.json`)
3. Global plugin-katalog (`~/.config/myassistant/plugins/`)
4. Prosjektpluginkatalog (`.myassistant/plugins/`)
...
MyAssistant kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En MyAssistant SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide MyAssistant.
...
:::note
Hvis du bruker MyAssistant-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre MyAssistant fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til MyAssistant:
...
`tool`-hjelperen lager et tilpasset verktøy som MyAssistant kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for MyAssistant sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://myassistant.ai/docs/sdk) for detaljer.
