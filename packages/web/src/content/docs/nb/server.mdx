---
title: Tjener
description: Samhandle med MyAssistant-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `myassistant serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en MyAssistant-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `myassistant.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `myassistant` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `myassistant serve` og `myassistant web`.
...
Når du kjører `myassistant` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk MyAssistant-serveren til å samhandle med MyAssistant programmatisk.
:::

Denne arkitekturen lar MyAssistant støtte flere klienter og lar deg samhandle med MyAssistant programmatisk.

Du kan kjøre `myassistant serve` for å starte en frittstående server. Hvis du har
MyAssistant TUI kjører, vil `myassistant serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av MyAssistant [IDE](/docs/ide) plugins.
...
myassistant-serveren viser følgende APIer.
