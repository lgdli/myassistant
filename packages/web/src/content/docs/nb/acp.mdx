---
title: ACP-støtte
description: Bruk myassistant i en hvilken som helst ACP-kompatibel editor.
---

MyAssistant støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke MyAssistant via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `myassistant acp`.

Kommandoen starter MyAssistant som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'MyAssistant'-agenten i AI Chat-agentvelgeren.
...
For å bruke MyAssistant som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke MyAssistant som ACP-agent for chat.
...
MyAssistant fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i MyAssistant-konfigurasjonen
