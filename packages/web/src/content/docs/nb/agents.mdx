...
Det er to typer agenter i MyAssistant; primære agenter og underagenter.
...
MyAssistant kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
MyAssistant kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

MyAssistant kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `myassistant.json`:
...
- Globalt: `~/.config/myassistant/agents/`
- Per prosjekt: `.myassistant/agents/`
...
Hvis ingen temperatur er spesifisert, bruker MyAssistant modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale MyAssistant-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i MyAssistant-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [MyAssistant Zen](/docs/zen), vil du bruke `myassistant/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `myassistant models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/myassistant).
:::
