...
Du kan definere MCP-servere i din [MyAssistant Config](https://opencode.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Hvis serveren kræver godkendelse, vil MyAssistant bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil MyAssistant opbevare tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
