---
title: Server
description: Interager med myassistant-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `myassistant serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en myassistant-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `myassistant.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `myassistant`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `myassistant serve` og `myassistant web`.
...
Når du kører `myassistant` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug myassistant-serveren til at interagere med myassistant programmatisk.
:::

Denne arkitektur lader myassistant understøtte flere klienter og lader dig interagere med myassistant programmatisk.

Du kan køre `myassistant serve` for at starte en selvstændig server. Hvis du har
myassistant TUI kørende, vil `myassistant serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af MyAssistant [IDE](/docs/ide) plugins.
...
MyAssistant-serveren viser følgende API'er.
