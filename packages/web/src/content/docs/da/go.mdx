...
MyAssistant Go er et lavprisabonnement — **$5 for din første måned**, derefter **$10/måned** — der giver dig pålidelig adgang til populære åbne kodningsmodeller.

:::note
MyAssistant Go er i øjeblikket i beta.
:::

Go fungerer som enhver anden udbyder i MyAssistant. Du abonnerer på MyAssistant Go og
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med MyAssistant.
:::
...
MyAssistant Go giver dig adgang til disse modeller for **$5 for din første måned**, derefter **$10/måned**.

---

## Sådan fungerer det

MyAssistant Go fungerer som enhver anden udbyder i MyAssistant.

1. Du logger ind på **<a href={console}>MyAssistant Zen</a>**, abonnerer på Go, og
   kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `MyAssistant Go`, og indsætter
   din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over tilgængelige modeller gennem Go.

:::note
Kun ét medlem per arbejdsområde kan abonnere på MyAssistant Go.
:::
...
MyAssistant Go inkluderer følgende grænser:
...
Dit [model id](/docs/config/#models) i din MyAssistant config
bruger formatet `myassistant-go/<model-id>`. For eksempel for Kimi K2.6, vil du
bruge `myassistant-go/kimi-k2.6` i din config.
...
Vi skabte MyAssistant Go for at:
...
4. Undgå **lock-in** ved også at lade dig bruge enhver anden udbyder med MyAssistant.
