---
title: Plugins
description: Skriv dine egne plugins for at udvide MyAssistant.
---

Plugins giver dig mulighed for at udvide MyAssistant ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre MyAssistants standardadfærd.
...
- `.myassistant/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/myassistant/node_modules/`.
...
1. Global konfiguration (`~/.config/myassistant/myassistant.json`)
2. Projektkonfiguration (`myassistant.json`)
3. Global plugin-mappe (`~/.config/myassistant/plugins/`)
4. Projekt plugin bibliotek (`.myassistant/plugins/`)
...
MyAssistant kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En myassistant SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide myassistant.
...
:::note
Hvis du bruger MyAssistant desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå myassistant i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til myassistant:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som myassistant kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for myassistant sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://myassistant.ai/docs/sdk) for detaljer.
