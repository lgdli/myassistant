...
For at fejlfinde problemer med MyAssistant, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\myassistant\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `myassistant --log-level DEBUG`.

---

## Lagring

myassistant gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/myassistant/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\myassistant`
...
MyAssistant Desktop kører en lokal MyAssistant-server (`myassistant-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `MyAssistant`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/myassistant/myassistant.jsonc` (eller `~/.config/myassistant/myassistant.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/myassistant/myassistant.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\myassistant\myassistant.jsonc`
...
MyAssistant kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/myassistant/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\myassistant\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så MyAssistant kan genopbygge den.

1. Afslut MyAssistant Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/myassistant`
- **Linux**: slet `~/.cache/myassistant` (eller kør `rm -rf ~/.cache/myassistant`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\myassistant`

3. Start MyAssistant Desktop på ny.

---

### Løs problemer med serverforbindelse

MyAssistant Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `myassistant.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver MyAssistant Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i MyAssistant.

---

### Meddelelser vises ikke

MyAssistant Desktop viser kun systemvarsler når:

- varsler er aktiveret for MyAssistant i dine OS-indstillinger, og
...
1. Afslut MyAssistant Desktop.
2. Find og slet disse filer (de findes i MyAssistant Desktop-appens datamappe):

- `myassistant.settings.dat` (skrivebordsstandardserver URL)
- `myassistant.global.dat` og `myassistant.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med MyAssistant:
...
For at finde ud af hvilke modeller du har adgang til, kør `myassistant models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\myassistant`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. myassistant installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\myassistant`

2. Start myassistant på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge myassistant til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
myassistant vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
