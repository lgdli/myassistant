---
title: CLI
description: MyAssistant CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MyAssistant CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
myassistant
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med MyAssistant programmatisk.
...
Start MyAssistant terminalbrugergrænsefladen.
...
MyAssistant CLI har også følgende kommandoer.

---

### agent

Administrer agenter i MyAssistant.
...
Tilslut en terminal til en allerede kørende MyAssistant backend-server startet via `serve` eller `web` kommandoer.

```bash
myassistant attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern MyAssistant backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `myassistant`) |
...
MyAssistant drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `myassistant auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/myassistant/auth.json`.

```bash
myassistant auth login
```

Når MyAssistant starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i MyAssistant.
...
Kør myassistant i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `myassistant serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Tilslut til en kørende myassistant-server (f.eks. http://localhost:4096)               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `myassistant`)   |
...
Start en hovedløs MyAssistant-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
myassistant serve
```

Dette starter en HTTP-server, der giver API-adgang til myassistant-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `myassistant`).
...
Administrator MyAssistant sessionsholder.
...
Liste over alle MyAssistant sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine MyAssistant-sessioner.
...
Importer sessionsdata fra en JSON fil eller MyAssistant del URL.

```bash
myassistant import <file>
```

Du kan importere fra en lokal fil eller en MyAssistant share URL.
...
Start en hovedløs MyAssistant-server med en webgrænseflade.

```bash
myassistant web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til MyAssistant via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `myassistant`).
...
Afinstaller MyAssistant og fjern alle relaterede filer.
...
Opdaterer MyAssistant til den nyeste version eller en specifik version.
...
myassistant CLI tager følgende globale flag.
...
MyAssistant kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `myassistant`) |
