...
Agentferdigheter lar MyAssistant oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
MyAssistant søker etter disse stedene:

- Prosjektkonfigurasjon: `.myassistant/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/myassistant/skills/<name>/SKILL.md`
...
For prosjektlokale stier går MyAssistant opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.myassistant/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/myassistant/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.myassistant/skills/git-release/SKILL.md` slik:
...
MyAssistant viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `myassistant.json`:
...
**For innebygde agenter** (i `myassistant.json`):
