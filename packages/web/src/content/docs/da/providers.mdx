---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i MyAssistant.
---

import config from "../../../../config.mjs"
export const console = config.console

MyAssistant bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i MyAssistant-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/myassistant/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i MyAssistant-konfigurationen.
...
MyAssistant Zen er en liste over modeller leveret af MyAssistant-teamet, som er blevet testet og verificeret til at fungere godt med MyAssistant. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med MyAssistant Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg myassistant og gå til [myassistant.ai/auth](https://myassistant.ai/auth).
...
Det fungerer som alle andre udbydere i MyAssistant og er helt valgfrit at bruge.

---

## MyAssistant Go

MyAssistant Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af MyAssistant-teamet, som er testet og verificeret til at fungere godt med MyAssistant.

1. Kør kommandoen `/connect` i TUI, vælg `MyAssistant Go`, og gå til [myassistant.ai/auth](https://myassistant.ai/zen).
...
Sådan bruger du Amazon Bedrock med MyAssistant:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører myassistant:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `myassistant.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i MyAssistant understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere myassistant til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at myassistant skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem myassistant-konfigurationen.
...
gemme tokenet i myassistant auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
MyAssistant bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse MyAssistant
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`myassistant.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [myassistant-gitlab-auth](https://www.npmjs.com/package/myassistant-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `myassistant.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med MyAssistant:
...
Sådan bruger du Google Vertex AI med MyAssistant:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører myassistant.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem myassistant, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [myassistant-helicone-session](https://github.com/H2Shami/myassistant-helicone-session) plugin for automatisk at logge hver MyAssistant-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver MyAssistant-samtale opført som en separat session.
...
Du kan konfigurere myassistant til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere myassistant til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere myassistant til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for MyAssistant. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/myassistant) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med MyAssistant:
...
3. Kopier API-nøglen til brug i MyAssistant.
...
6. **Vigtigt**: Før du bruger skymodeller i MyAssistant, skal du hente modelinformationen lokalt:
...
MyAssistant Zen er en liste over testede og verificerede modeller leveret af MyAssistant-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>MyAssistant Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **MyAssistant Zen**.

   ```txt
   /connect
   ```

3. Indtast MyAssistant API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem myassistant-konfigurationen.
...
5. Du kan også tilpasse dem gennem myassistant-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med MyAssistant:
...
Du kan også tilpasse modeller gennem myassistant-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med myassistant. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `myassistant.json`-filen i projektkataloget:
...
`limit`-felterne lader MyAssistant forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `myassistant auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek myassistant-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i myassistant-konfigurationen.
