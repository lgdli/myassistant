...
Med MyAssistant kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger MyAssistant vårt eget `myassistant`-tema.
...
MyAssistant støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/myassistant/themes/*.json` eller `$XDG_CONFIG_HOME/myassistant/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.myassistant/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.myassistant/themes/*.json`
