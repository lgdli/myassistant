---
title: Dodaci
description: Napišite vlastite dodatke za proširenje MyAssistant.
---

Dodaci vam omogućavaju da proširite MyAssistant spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja MyAssistant.
...
- `.myassistant/plugins/` - Dodaci na nivou projekta
- `~/.config/myassistant/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/myassistant/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/myassistant/myassistant.json`)
2. Konfiguracija projekta (`myassistant.json`)
3. Globalni direktorij dodataka (`~/.config/myassistant/plugins/`)
4. Direktorij dodataka projekta (`.myassistant/plugins/`)
...
MyAssistant pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje MyAssistant.
...
Spriječite myassistant da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u myassistant:
...
Pomoćnik `tool` kreira prilagođeni alat koji myassistant može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za myassistant zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://myassistant.ai/docs/sdk) za detalje.
