...
Možete dodati vanjske alate u MyAssistant koristeći _Model Context Protocol_, ili MCP. MyAssistant podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [MyAssistant Config](https://myassistant.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/myassistant`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
MyAssistant automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, MyAssistant će:
...
Ako server zahtijeva autentifikaciju, MyAssistant će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `myassistant mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, MyAssistant sigurno čuva tokene u `~/.local/share/myassistant/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u MyAssistant, zajedno s ugrađenim alatima. Možete njima upravljati kroz MyAssistant konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete MyAssistant sa vašim Sentry nalogom.
