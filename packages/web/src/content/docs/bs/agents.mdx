...
Postoje dvije vrste agenata u MyAssistant: primarni agenti i podagenti.
...
MyAssistant dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
MyAssistant dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

MyAssistant dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `myassistant.json`:
...
- Globalno: `~/.config/myassistant/agents/`
- Po projektu: `.myassistant/agents/`
...
Ako temperatura nije navedena, MyAssistant koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu MyAssistant konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj MyAssistant konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [MyAssistant Zen](/docs/zen), koristili biste `myassistant/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `myassistant models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/myassistant).
:::
