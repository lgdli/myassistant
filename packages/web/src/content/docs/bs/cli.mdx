---
title: CLI
description: MyAssistant CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MyAssistant CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
myassistant
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa MyAssistant.
...
Pokrenite MyAssistant terminalski korisnički interfejs.
...
MyAssistant CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte MyAssistant agentima.
...
Priključite terminal na već pokrenut MyAssistant backend server pokrenut putem `serve` ili `web` naredbi.

```bash
myassistant attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim MyAssistant backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `myassistant`) |
...
MyAssistant pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `myassistant auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/myassistant/auth.json`.

```bash
myassistant auth login
```

Kada se MyAssistant pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u MyAssistant.
...
Pokrenite MyAssistant u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `myassistant serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti MyAssistant server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `myassistant`) |
...
Pokrenite MyAssistant headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
myassistant serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti MyAssistant-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `myassistant`).
...
Upravljajte MyAssistant sesijama.
...
Navedite sve MyAssistant sesije.
...
Prikaži statistiku upotrebe tokena i troškova za vaše MyAssistant sesije.
...
Uvezite podatke sesije iz JSON datoteke ili MyAssistant dijeljenog URL-a.

```bash
myassistant import <file>
```

Možete uvesti iz lokalne datoteke ili MyAssistant dijeljenog URL-a.
...
Pokrenite MyAssistant headless server sa web interfejsom.

```bash
myassistant web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup MyAssistant-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `myassistant`).
...
Deinstalirajte MyAssistant i uklonite sve povezane datoteke.
...
Ažurira MyAssistant na najnoviju verziju ili određenu verziju.
...
MyAssistant CLI prihvata sljedeće globalne zastavice.
...
MyAssistant se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `myassistant`) |
