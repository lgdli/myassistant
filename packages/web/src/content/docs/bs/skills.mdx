...
Agent skills omogucavaju MyAssistant da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
MyAssistant pretrazuje ove lokacije:

- Konfiguracija projekta: `.myassistant/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/myassistant/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, MyAssistant ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.myassistant/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/myassistant/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.myassistant/skills/git-release/SKILL.md` ovako:
...
MyAssistant navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `myassistant.json`:
...
**Za ugradene agente** (u `myassistant.json`):
