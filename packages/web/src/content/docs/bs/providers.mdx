---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u MyAssistant.
---

import config from "../../../../config.mjs"
export const console = config.console

MyAssistant koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj MyAssistant konfiguraciji.
...
u `~/.local/share/myassistant/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem MyAssistant
config.
...
MyAssistant Zen je lista modela koje je obezbedio MyAssistant tim koji su bili
testirano i potvrđeno da dobro radi sa MyAssistant. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa MyAssistant Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite myassistant i idite na [myassistant.ai/auth](https://myassistant.ai/auth).
...
Radi kao i svaki drugi provajder u MyAssistant i potpuno je opcionalan za korištenje.

---

## MyAssistant Go

MyAssistant Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža MyAssistant tim i koji su testirani i verificirani da dobro rade s MyAssistant-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `MyAssistant Go` i idite na [myassistant.ai/auth](https://myassistant.ai/zen).
...
Da biste koristili Amazon Bedrock s MyAssistant:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete myassistant:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `myassistant.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u MyAssistant.
:::
...
Možete konfigurirati myassistant za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi myassistant ispravno radio.
   :::
...
Također možete dodati modele kroz svoju myassistant konfiguraciju.
...
da pohrani token u myassistant auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
MyAssistant koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate MyAssistant
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`myassistant.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [myassistant-gitlab-auth](https://www.npmjs.com/package/myassistant-gitlab-auth).

##### Konfiguracija

Prilagodite putem `myassistant.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s myassistant:
...
Za korištenje Google Vertex AI s MyAssistant:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće myassistant.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [myassistant-helicone-session](https://github.com/H2Shami/myassistant-helicone-session) da automatski prijavite svaki MyAssistant razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki MyAssistant razgovor naveden kao zasebna sesija.
...
Možete konfigurirati myassistant za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati myassistant za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati myassistant za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za MyAssistant. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/myassistant) za detalje.
:::
...
Da biste koristili Ollama Cloud s MyAssistant:
...
3. Kopirajte API ključ za korištenje u MyAssistant.
...
6. **Važno**: Prije upotrebe modela oblaka u MyAssistant, morate lokalno povući informacije o modelu:
...
MyAssistant Zen je lista testiranih i verifikovanih modela koju je obezbedio MyAssistant tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>MyAssistant Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **MyAssistant Zen**.

```txt
   /connect
```

3. Unesite svoj MyAssistant API ključ.
...
Također možete dodati dodatne modele putem vaše myassistant konfiguracije.
...
5. Također ih možete prilagoditi putem vaše myassistant konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju myassistant konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s myassistant-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `myassistant.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju MyAssistant da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `myassistant auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite MyAssistant konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj myassistant konfiguraciji.
