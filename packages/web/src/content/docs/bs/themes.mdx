...
U MyAssistant mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, MyAssistant koristi nasu `myassistant` temu.
...
MyAssistant dolazi sa vise ugradenih tema.
...
- Zele da MyAssistant odgovara izgledu njihovog terminala
...
MyAssistant podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/myassistant/themes/*.json` ili `$XDG_CONFIG_HOME/myassistant/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.myassistant/themes/*.json`
4. **Trenutni radni direktorij** - `./.myassistant/themes/*.json`
