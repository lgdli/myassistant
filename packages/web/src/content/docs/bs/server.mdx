---
title: Server
description: Komunicirajte s myassistant serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `myassistant serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji myassistant klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `myassistant.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `myassistant`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `myassistant serve` i za `myassistant web`.
...
Kada pokrenete `myassistant`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite myassistant server za programsku interakciju sa myassistant-om.
:::

Ova arhitektura omogucava myassistant podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `myassistant serve` da startate standalone server. Ako je myassistant TUI vec pokrenut, `myassistant serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste MyAssistant [IDE](/docs/ide) pluginovi.
...
myassistant server izlaže sljedece API-je.
