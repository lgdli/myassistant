...
Da biste otklonili probleme s MyAssistant, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\myassistant\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `myassistant --log-level DEBUG`.

---

## Pohrana

MyAssistant pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/myassistant/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\myassistant`
...
MyAssistant Desktop pokreće lokalni MyAssistant server (`myassistant-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `MyAssistant` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/myassistant/myassistant.jsonc` (ili `~/.config/myassistant/myassistant.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/myassistant/myassistant.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\myassistant\myassistant.jsonc`
...
MyAssistant također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/myassistant/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\myassistant\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga MyAssistant mogao ponovo izgraditi.

1. Potpuno zatvorite MyAssistant Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/myassistant`
- **Linux**: obrišite `~/.cache/myassistant` (ili pokrenite `rm -rf ~/.cache/myassistant`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\myassistant`

3. Ponovo pokrenite MyAssistant Desktop.

---

### Rješavanje problema sa vezom na serveru

MyAssistant Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `myassistant.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, MyAssistant Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa MyAssistant karakteristikama.

---

### Obavještenja se ne prikazuju

MyAssistant Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za MyAssistant u postavkama vašeg OS-a, i
...
1. Zatvorite MyAssistant Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka MyAssistant Desktop aplikacije):

- `myassistant.settings.dat` (URL zadanog servera za desktop)
- `myassistant.global.dat` i `myassistant.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s MyAssistant:
...
Da saznate kojim modelima imate pristup, pokrenite `myassistant models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\myassistant`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. MyAssistant dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\myassistant`

2. Ponovo pokrenite MyAssistant da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti MyAssistant da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
MyAssistant će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
