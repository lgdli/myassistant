---
title: Konfiguracija
description: Korištenje MyAssistant JSON konfiguracije.
---

Možete konfigurirati MyAssistant koristeći JSON konfiguracijski fajl.

---

## Format

MyAssistant podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/myassistant`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/myassistant/myassistant.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`myassistant.json` u projektu) - postavke specifične za projekat
5. **`.myassistant` direktoriji** - agenti, komande, dodaci
...
:::note
Direktoriji `.myassistant` i `~/.config/myassistant` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/myassistant` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu MyAssistant konfiguraciju u `~/.config/myassistant/myassistant.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/myassistant/tui.json`.
...
Dodajte `myassistant.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se MyAssistant pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.myassistant` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.myassistant`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`myassistant.ai/config.json`**](https://myassistant.ai/config.json).

TUI konfiguracija koristi [**`myassistant.ai/tui.json`**](https://myassistant.ai/tui.json).
...
Stari `theme`, `keybinds`, i `tui` ključevi u `myassistant.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `myassistant serve` i `myassistant web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš MyAssistant server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `myassistant.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj MyAssistant konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, MyAssistant pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/myassistant/agents/` ili `.myassistant/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, MyAssistant će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`myassistant run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/myassistant/commands/` ili `.myassistant/commands/`. [Saznajte više ovdje](/docs/commands).
...
MyAssistant će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, MyAssistant **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju MyAssistant sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.myassistant/plugins/` ili `~/.config/myassistant/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite MyAssistant da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
