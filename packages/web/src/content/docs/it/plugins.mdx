---
title: Plugin
description: Scrivi plugin per estendere MyAssistant.
---

I plugin ti permettono di estendere MyAssistant agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di MyAssistant.
...
- `.myassistant/plugins/` - plugin a livello progetto
- `~/.config/myassistant/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/myassistant/node_modules/`.
...
1. Config globale (`~/.config/myassistant/myassistant.json`)
2. Config di progetto (`myassistant.json`)
3. Directory plugin globale (`~/.config/myassistant/plugins/`)
4. Directory plugin di progetto (`.myassistant/plugins/`)
...
MyAssistant esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di myassistant per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere myassistant.
...
:::note
Se usi l'app desktop di MyAssistant, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a myassistant di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a myassistant:
...
L'helper `tool` crea uno strumento personalizzato che myassistant può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in myassistant insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://myassistant.ai/docs/sdk) per i dettagli.
