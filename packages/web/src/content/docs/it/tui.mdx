---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di MyAssistant.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MyAssistant fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire MyAssistant avvia la TUI per la directory corrente.
...
Quando usi la TUI di MyAssistant, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a MyAssistant. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da MyAssistant. _Alias_: `/quit`, `/q`
...
Questo è separato da `myassistant.json`, che configura il comportamento di server/runtime.
