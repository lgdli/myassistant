...
Le skill degli agenti permettono a MyAssistant di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
MyAssistant cerca in queste posizioni:

- Config di progetto: `.myassistant/skills/<name>/SKILL.md`
- Config globale: `~/.config/myassistant/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, MyAssistant risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.myassistant/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/myassistant/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.myassistant/skills/git-release/SKILL.md` cosi':
...
MyAssistant elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `myassistant.json`:
...
**Per agenti integrati** (in `myassistant.json`):
