---
title: CLI
description: Opzioni e comandi della CLI di MyAssistant.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di MyAssistant, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
myassistant
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con MyAssistant in modo programmatico.
...
Avvia l'interfaccia testuale di MyAssistant (interfaccia utente da terminale).
...
La CLI di MyAssistant include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per MyAssistant.
...
Collega un terminale a un backend server di MyAssistant già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
myassistant attach [url]
```

Questo consente di usare la TUI con un backend MyAssistant remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `myassistant`) |
...
MyAssistant si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `myassistant auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/myassistant/auth.json`.

```bash
myassistant auth login
```

Quando MyAssistant si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in MyAssistant.
...
Esegui myassistant in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `myassistant serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server myassistant in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `myassistant`) |
...
Avvia un server MyAssistant headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
myassistant serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di myassistant senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `myassistant`).
...
Gestisci le sessioni MyAssistant.
...
Elenca tutte le sessioni MyAssistant.
...
Mostra statistiche di utilizzo token e costo per le sessioni MyAssistant.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione MyAssistant.

```bash
myassistant import <file>
```

Puoi importare da un file locale o da un URL di condivisione MyAssistant.
...
Avvia un server MyAssistant headless con interfaccia web.

```bash
myassistant web
```

Avvia un server HTTP e apre un browser per accedere a MyAssistant tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `myassistant`).
...
Disinstalla MyAssistant e rimuove tutti i file correlati.
...
Aggiorna myassistant all'ultima versione o a una versione specifica.
...
La CLI di myassistant accetta i seguenti flag globali.
...
MyAssistant può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `myassistant`)     |
