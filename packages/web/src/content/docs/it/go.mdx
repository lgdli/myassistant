...
MyAssistant Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

:::note
MyAssistant Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in MyAssistant. Ti abboni a MyAssistant Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare MyAssistant.
...
MyAssistant Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

MyAssistant Go funziona come qualsiasi altro provider in MyAssistant.

1. Accedi a **<a href={console}>MyAssistant Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `MyAssistant Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a MyAssistant Go.
:::
...
MyAssistant Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua MyAssistant config
utilizza il formato `myassistant-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`myassistant-go/kimi-k2.6` nella tua configurazione.
...
Abbiamo creato MyAssistant Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con MyAssistant.
