...
Puoi aggiungere strumenti esterni a MyAssistant usando il _Model Context Protocol_, o MCP. MyAssistant supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [MyAssistant Config](https://myassistant.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/myassistant`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
MyAssistant gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, MyAssistant:
...
Se il server richiede autenticazione, MyAssistant ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `myassistant mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, MyAssistant memorizzera' i token in modo sicuro in `~/.local/share/myassistant/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in MyAssistant insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione MyAssistant come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare MyAssistant al tuo account Sentry.
