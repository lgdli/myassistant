...
In MyAssistant esistono due tipi di agenti: agenti primari e subagenti.
...
MyAssistant include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
MyAssistant include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

MyAssistant include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `myassistant.json`:
...
- Globale: `~/.config/myassistant/agents/`
- Per progetto: `.myassistant/agents/`
...
Se non specifichi una temperature, MyAssistant usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di MyAssistant sia per la config specifica del progetto.
...
L'ID modello nella configurazione MyAssistant usa il formato `provider/model-id`. Per esempio, se stai usando [MyAssistant Zen](/docs/zen), useresti `myassistant/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `myassistant models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/myassistant).
:::
