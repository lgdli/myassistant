...
With MyAssistant you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, MyAssistant uses our own `myassistant` theme.
...
MyAssistant comes with several built-in themes.
...
- Want MyAssistant to match their terminal's appearance
...
MyAssistant supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/myassistant/themes/*.json` or `$XDG_CONFIG_HOME/myassistant/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.myassistant/themes/*.json`
4. **Current working directory** - Defined in `./.myassistant/themes/*.json`
