...
You can define MCP servers in your [MyAssistant Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
If the server requires authentication, MyAssistant will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, MyAssistant will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
