...
You can add external tools to MyAssistant using the _Model Context Protocol_, or MCP. MyAssistant supports both local and remote servers.
...
You can define MCP servers in your [MyAssistant Config](https://myassistant.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/myassistant` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
MyAssistant automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, MyAssistant will:
...
If the server requires authentication, MyAssistant will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `myassistant mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, MyAssistant will store the tokens securely in `~/.local/share/myassistant/mcp-auth.json`.
...
Your MCPs are available as tools in MyAssistant, alongside built-in tools. So you can manage them through the MyAssistant config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect MyAssistant to your Sentry account.
