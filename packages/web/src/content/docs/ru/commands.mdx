...
Создайте `.myassistant/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию myassistant или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем myassistant [config](/docs/config):
...
- Глобальный: `~/.config/myassistant/commands/`
- Для каждого проекта: `.myassistant/commands/`
...
myassistant включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
