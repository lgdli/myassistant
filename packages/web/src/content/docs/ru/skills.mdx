...
Навыки агента позволяют myassistant обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
myassistant выполняет поиск в следующих местах:

- Конфигурация проекта: `.myassistant/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/myassistant/skills/<name>/SKILL.md`.
...
Для локальных путей проекта myassistant переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.myassistant/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/myassistant/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.myassistant/skills/git-release/SKILL.md` следующим образом:
...
myassistant перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `myassistant.json`:
...
**Для встроенных агентов** (в формате `myassistant.json`):
