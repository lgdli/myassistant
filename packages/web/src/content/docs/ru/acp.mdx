---
title: Поддержка ACP
description: Используйте myassistant в любом ACP-совместимом редакторе.
---

myassistant поддерживает [Agent Client Protocol](https://agentclientprotocol.com) (ACP), что позволяет использовать его непосредственно в совместимых редакторах и IDE.
...
Чтобы использовать myassistant через ACP, настройте свой редактор для запуска команды `myassistant acp`.

Команда запускает myassistant как ACP-совместимый подпроцесс, который взаимодействует с вашим редактором через JSON-RPC через stdio.
...
Чтобы открыть его, используйте новый агент myassistant в селекторе агентов AI Chat.
...
Чтобы использовать myassistant в качестве агента ACP в [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), добавьте в конфигурацию Neovim следующее:
...
Эта конфигурация настраивает CodeCompanion для использования myassistant в качестве агента ACP для чата.
...
myassistant через ACP работает так же, как и в терминале. Поддерживаются все функции:
...
- Серверы MCP, настроенные в вашей конфигурации myassistant
