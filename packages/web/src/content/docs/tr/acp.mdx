---
title: ACP Desteği
description: myassistant'u ACP uyumlu herhangi bir editörde kullanın.
---

myassistant, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
myassistant'u ACP aracılığıyla kullanmak için düzenleyicinizi `myassistant acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, myassistant'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'MyAssistant' ajanını kullanın.
...
myassistant'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı myassistant'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
myassistant, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- myassistant yapılandırmanızda yapılandırılmış MCP sunucuları
