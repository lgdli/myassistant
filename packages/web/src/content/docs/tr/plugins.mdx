---
title: Eklentiler
description: MyAssistant'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek MyAssistant'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya MyAssistant'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.myassistant/plugins/` - Proje düzeyinde eklentiler
- `~/.config/myassistant/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/myassistant/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/myassistant/myassistant.json`)
2. Project config (`myassistant.json`)
3. Global eklenti dizini (`~/.config/myassistant/plugins/`)
4. Proje eklenti dizini (`.myassistant/plugins/`)
...
MyAssistant bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir MyAssistant SDK istemcisi.
...
MyAssistant'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
MyAssistant masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
MyAssistant'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca MyAssistant'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, MyAssistant'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra MyAssistant'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://myassistant.ai/docs/sdk) bakın.
