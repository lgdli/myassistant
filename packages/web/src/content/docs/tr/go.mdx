...
MyAssistant Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
MyAssistant Go şu anda beta olarak mevcut.
:::

Go, MyAssistant'daki diğer sağlayıcılar gibi çalışır. MyAssistant Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve MyAssistant'u kullanmak için bunu kullanmanıza gerek yoktur.
...
MyAssistant Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

MyAssistant Go, MyAssistant'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>MyAssistant Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `MyAssistant Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye MyAssistant Go'ya abone olabilir.
:::
...
MyAssistant Go aşağıdaki limitleri içerir:
...
MyAssistant yapılandırmanızdaki [model id](/docs/config/#models) formatı `myassistant-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `myassistant-go/kimi-k2.6` kullanmalısınız.
...
MyAssistant Go'yu şu amaçlarla oluşturduk:
...
4. MyAssistant ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
