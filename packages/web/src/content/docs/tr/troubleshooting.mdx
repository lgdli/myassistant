...
myassistant ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\myassistant\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `myassistant --log-level DEBUG`.

---

## Depolama

myassistant, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/myassistant/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\myassistant` yapistirin
...
myassistant Desktop arka planda yerel bir myassistant sunucusu (`myassistant-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `myassistant` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/myassistant/myassistant.jsonc` (veya `~/.config/myassistant/myassistant.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/myassistant/myassistant.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\myassistant\myassistant.jsonc` yapistirin
...
myassistant diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/myassistant/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\myassistant\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), myassistant'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. myassistant Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/myassistant`
- **Linux**: `~/.cache/myassistant` dizinini silin (veya `rm -rf ~/.cache/myassistant` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\myassistant` yapistirin

3. myassistant Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

myassistant Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`myassistant.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta myassistant Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, myassistant ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

myassistant Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda myassistant icin bildirimler etkinse
...
1. myassistant Desktop'u kapatin
2. Su dosyalari bulun ve silin (myassistant Desktop uygulama veri dizininde yer alirlar):

- `myassistant.settings.dat` (desktop varsayilan sunucu URL'si)
- `myassistant.global.dat` ve `myassistant.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
myassistant ile ilgili bir sorun yasiyorsaniz:
...
Erisiminiz olan modelleri gormek icin `myassistant models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\myassistant`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. myassistant, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\myassistant`

2. En guncel provider paketlerini yeniden kurmak icin myassistant'u yeniden baslatin

Bu, myassistant'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
myassistant, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
