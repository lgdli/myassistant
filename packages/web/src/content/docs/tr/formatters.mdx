...
description: myassistant dile özgü biçimlendiriciler kullanır.
---

myassistant, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

myassistant, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, myassistant bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

myassistant bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri myassistant yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
