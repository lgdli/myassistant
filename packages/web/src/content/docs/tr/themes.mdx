...
myassistant ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak myassistant kendi `myassistant` temasını kullanır.
...
myassistant birden fazla yerleşik temayla gelir.
...
- myassistant'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
myassistant, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/myassistant/themes/*.json` veya `$XDG_CONFIG_HOME/myassistant/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.myassistant/themes/*.json`
4. **Geçerli çalışma dizini** - `./.myassistant/themes/*.json`
