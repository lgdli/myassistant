...
Ajan becerileri, myassistant'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
myassistant su konumlari tarar:

- Proje konfigurasyonu: `.myassistant/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/myassistant/skills/<name>/SKILL.md`
...
Proje yerel yollarinda myassistant, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.myassistant/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/myassistant/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.myassistant/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
myassistant, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`myassistant.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`myassistant.json` icinde):
