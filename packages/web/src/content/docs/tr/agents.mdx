...
myassistant'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
myassistant, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
myassistant, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

myassistant iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `myassistant.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.myassistant/agents/`
...
Sıcaklık belirtilmezse myassistant modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global myassistant yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
myassistant yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [MyAssistant Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `myassistant/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `myassistant models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/myassistant).
:::
