...
_Model Bağlam Protokolü_ veya MCP kullanarak myassistant'a harici araçlar ekleyebilirsiniz. myassistant hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [myassistant Config](https://myassistant.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/myassistant` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
myassistant, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde myassistant şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa myassistant, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `myassistant mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından myassistant, belirteçleri `~/.local/share/myassistant/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra myassistant'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi myassistant yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve myassistant'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
