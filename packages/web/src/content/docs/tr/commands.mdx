...
`.myassistant/commands/test.md` oluştur:
...
myassistant işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

myassistant [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/myassistant/commands/`
- Proje başına: `.myassistant/commands/`
...
myassistant, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
