---
title: Sağlayıcılar
description: myassistant'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

myassistant, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı myassistant yapılandırmanızda yapılandırın.
...
`~/.local/share/myassistant/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları myassistant'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
MyAssistant Zen, myassistant ekibi tarafından sağlanan modellerin bir listesidir.
myassistant ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz MyAssistant Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, myassistant'u seçin ve [myassistant.ai/auth](https://myassistant.ai/auth)'ye gidin.
...
myassistant'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## MyAssistant Go

MyAssistant Go, myassistant ile iyi çalıştığı test edilmiş ve doğrulanmış, myassistant ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `MyAssistant Go`'yu seçin ve [myassistant.ai/auth](https://myassistant.ai/zen) adresine gidin.
...
Amazon Bedrock'u myassistant ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   myassistant'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `myassistant.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi myassistant'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
myassistant'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   myassistant'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in myassistant auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
myassistant, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. myassistant'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`myassistant.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [myassistant-gitlab-auth](https://www.npmjs.com/package/myassistant-gitlab-auth) ana sayfasında.

##### Yapılandırma

`myassistant.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi myassistant'la kullanmak için:
...
Google Vertex AI'yi myassistant ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları myassistant çalıştırılırken ayarlayın.
...
Helicone'un myassistant aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her myassistant görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [myassistant-helicone-session](https://github.com/H2Shami/myassistant-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her myassistant görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
myassistant'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla myassistant'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla myassistant'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini myassistant için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/myassistant)'a bakın.
:::
...
Ollama Cloud'u myassistant ile kullanmak için:
...
3. myassistant'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: myassistant'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
MyAssistant Zen, myassistant ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>MyAssistant Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **MyAssistant Zen**'i arayın.

   ```txt
   /connect
   ```

3. myassistant API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   myassistant yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca myassistant yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri myassistant yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı myassistant ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `myassistant.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları myassistant'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `myassistant auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için myassistant yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, myassistant yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
