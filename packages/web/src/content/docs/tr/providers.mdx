...
MyAssistant Zen, opencode ekibi tarafından sağlanan modellerin bir listesidir.
...
MyAssistant Go, opencode ile iyi çalıştığı test edilmiş ve doğrulanmış, opencode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `MyAssistant Go`'yu seçin ve [opencode.ai/auth](https://opencode.ai/zen) adresine gidin.
...
MyAssistant Zen, opencode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).
