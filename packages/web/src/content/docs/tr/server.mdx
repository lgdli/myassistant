---
title: Sunucu
description: myassistant sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`myassistant serve` komutu, myassistant istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `myassistant.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OPENCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `myassistant` değeridir; değiştirmek isterseniz `OPENCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `myassistant serve` hem de `myassistant web` için geçerlidir.
...
`myassistant` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
myassistant ile programatik etkileşim için myassistant sunucusunu kullanın.
:::

Bu mimari, myassistant'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`myassistant serve` ile bağımsız bir sunucu başlatabilirsiniz. myassistant TUI açıksa, `myassistant serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum myassistant [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
myassistant sunucusu aşağıdaki API'leri sunar.
