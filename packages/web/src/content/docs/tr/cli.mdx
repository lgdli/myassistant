---
title: CLI
description: myassistant CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

myassistant CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
myassistant
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, myassistant ile programlı olarak etkileşim kurmanıza olanak tanır.
...
myassistant terminal kullanıcı arayüzünü başlatır.
...
myassistant CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

myassistant için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir myassistant arka uç sunucusuna bir terminal ekleyin.

```bash
myassistant attach [url]
```

Bu, TUI öğesinin uzak bir myassistant arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `myassistant`) |
...
myassistant, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `myassistant auth login` kullanabilirsiniz. Bu `~/.local/share/myassistant/auth.json`'de saklanıyor.

```bash
myassistant auth login
```

myassistant başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları myassistant'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek myassistant'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `myassistant serve` örneğine de ekleyebilirsiniz:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir myassistant sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `myassistant`) |
...
API erişimi için headless bir myassistant sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
myassistant serve
```

Bu, TUI arayüzü olmadan myassistant işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `myassistant` şeklindedir).
...
myassistant oturumlarını yönetin.
...
Tüm myassistant oturumlarını listeleyin.
...
myassistant oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya myassistant paylaşımından URL oturum verilerini içe aktarın.

```bash
myassistant import <file>
```

Yerel bir dosyadan veya myassistant paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir myassistant sunucusu başlatın.

```bash
myassistant web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla myassistant'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `myassistant` şeklindedir).
...
myassistant'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
myassistant'u en son sürüme veya belirli bir sürüme günceller.
...
myassistant CLI aşağıdaki global bayrakları destekler.
...
myassistant ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `myassistant`) |
