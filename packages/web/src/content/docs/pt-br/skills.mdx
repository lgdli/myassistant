...
As habilidades do agente permitem que o myassistant descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O myassistant pesquisa nesses locais:

- Configuração do projeto: `.myassistant/skills/<name>/SKILL.md`
- Configuração global: `~/.config/myassistant/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o myassistant sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.myassistant/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/myassistant/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.myassistant/skills/git-release/SKILL.md` assim:
...
O myassistant lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `myassistant.json`:
...
**Para agentes embutidos** (em `myassistant.json`):
