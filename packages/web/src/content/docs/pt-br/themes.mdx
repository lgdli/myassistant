...
Com o myassistant, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o myassistant usa nosso próprio tema `myassistant`.
...
O myassistant vem com vários temas embutidos.
...
- Querem que o myassistant corresponda à aparência do seu terminal
...
O myassistant suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/myassistant/themes/*.json` ou `$XDG_CONFIG_HOME/myassistant/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.myassistant/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.myassistant/themes/*.json`
