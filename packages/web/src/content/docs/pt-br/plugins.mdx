---
title: Plugins
description: Escreva seus próprios plugins para estender o myassistant.
---

Plugins permitem que você estenda o myassistant conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do myassistant.
...
- `.myassistant/plugins/` - Plugins em nível de projeto
- `~/.config/myassistant/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/myassistant/node_modules/`.
...
1. Configuração global (`~/.config/myassistant/myassistant.json`)
2. Configuração do projeto (`myassistant.json`)
3. Diretório de plugins global (`~/.config/myassistant/plugins/`)
4. Diretório de plugins do projeto (`.myassistant/plugins/`)
...
O myassistant executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do myassistant para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o myassistant.
...
:::note
Se você estiver usando o aplicativo desktop myassistant, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o myassistant de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao myassistant:
...
O helper `tool` cria uma ferramenta personalizada que o myassistant pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o myassistant junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://myassistant.ai/docs/sdk) para detalhes.
