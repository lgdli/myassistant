---
title: CLI
description: Opções e comandos da CLI do myassistant.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do myassistant, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
myassistant
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o myassistant programaticamente.
...
Inicie a interface de usuário do terminal do myassistant.
...
A CLI do myassistant também possui os seguintes comandos.

---

### agent

Gerencie agentes para o myassistant.
...
Anexe um terminal a um servidor backend do myassistant já em execução, iniciado via comandos `serve` ou `web`.

```bash
myassistant attach [url]
```

Isso permite usar o TUI com um backend myassistant remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `myassistant`) |
...
O myassistant é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `myassistant auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/myassistant/auth.json`.

```bash
myassistant auth login
```

Quando o myassistant é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no myassistant.
...
Execute o myassistant em modo não interativo passando um prompt diretamente.
...
myassistant run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `myassistant serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
myassistant run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor myassistant em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `myassistant`) |
...
Inicie um servidor myassistant sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
myassistant serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do myassistant sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `myassistant`).
...
Gerencie sessões do myassistant.
...
Liste todas as sessões do myassistant.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do myassistant.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do myassistant.

```bash
myassistant import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do myassistant.
...
Inicie um servidor myassistant sem cabeça com uma interface web.

```bash
myassistant web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o myassistant através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `myassistant`).
...
Desinstale o myassistant e remova todos os arquivos relacionados.
...
Atualiza o myassistant para a versão mais recente ou uma versão específica.
...
A CLI do myassistant aceita as seguintes flags globais.
...
O myassistant pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `myassistant`) |
