---
title: Servidor
description: Interaja com o servidor myassistant via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `myassistant serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente myassistant pode usar.
...
```bash
myassistant serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `myassistant.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
myassistant serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `myassistant`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `myassistant serve` quanto ao `myassistant web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password myassistant serve
```
...
Quando você executa `myassistant`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor myassistant para interagir com o myassistant programaticamente.
:::

Essa arquitetura permite que o myassistant suporte múltiplos clientes e permite que você interaja com o myassistant programaticamente.

Você pode executar `myassistant serve` para iniciar um servidor autônomo. Se você tiver o TUI do myassistant em execução, `myassistant serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do myassistant [IDE](/docs/ide).
...
O servidor myassistant expõe as seguintes APIs.
