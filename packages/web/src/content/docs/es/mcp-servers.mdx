...
Puede definir servidores MCP en su [MyAssistant Config](https://opencode.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Si el servidor requiere autenticación, MyAssistant le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `opencode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, MyAssistant almacenará los tokens de forma segura en `~/.local/share/opencode/mcp-auth.json`.
