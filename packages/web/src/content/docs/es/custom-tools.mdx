...
description: Cree herramientas que LLM pueda llamar en myassistant.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de myassistant como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.myassistant/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/myassistant/tools/`.
...
```ts title=".myassistant/tools/database.ts" {1}
import { tool } from "@myassistant-ai/plugin"
...
```ts title=".myassistant/tools/math.ts"
import { tool } from "@myassistant-ai/plugin"
...
```ts title=".myassistant/tools/bash.ts"
import { tool } from "@myassistant-ai/plugin"
...
```ts title=".myassistant/tools/project.ts" {8}
import { tool } from "@myassistant-ai/plugin"
...
```python title=".myassistant/tools/add.py"
import sys
...
```ts title=".myassistant/tools/python-add.ts" {10}
import { tool } from "@myassistant-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".myassistant/tools/add.py")
