---
title: CLI
description: Opciones y comandos de la CLI de MyAssistant.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de MyAssistant de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
myassistant
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con MyAssistant mediante programación.
...
Inicie la interfaz de usuario del terminal MyAssistant.
...
La CLI de MyAssistant también tiene los siguientes comandos.

---

### agent

Administrar agentes para MyAssistant.
...
Conecte una terminal a un servidor backend MyAssistant que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
myassistant attach [url]
```

Esto permite usar TUI con un backend remoto MyAssistant. Por ejemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `myassistant`) |
...
MyAssistant funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `myassistant auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/myassistant/auth.json`.

```bash
myassistant auth login
```

Cuando se inicia MyAssistant, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en MyAssistant.
...
Ejecute myassistant en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `myassistant serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor myassistant en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `myassistant`) |
...
Inicie un servidor MyAssistant sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
myassistant serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad myassistant sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `myassistant`).
...
Administrar MyAssistant sesiones.
...
Enumere todas las sesiones MyAssistant.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones MyAssistant.
...
Importe datos de sesión desde un archivo JSON o una URL compartida MyAssistant.

```bash
myassistant import <file>
```

Puede importar desde un archivo local o una URL compartida MyAssistant.
...
Inicie un servidor MyAssistant sin cabeza con una interfaz web.

```bash
myassistant web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a MyAssistant a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `myassistant`).
...
Desinstale MyAssistant y elimine todos los archivos relacionados.
...
Actualiza myassistant a la última versión o a una versión específica.
...
La CLI de MyAssistant toma las siguientes banderas globales.
...
MyAssistant se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `myassistant`) |
