...
Con MyAssistant puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, MyAssistant usa nuestro propio tema `myassistant`.
...
MyAssistant viene con varios temas integrados.
...
- Quiere que MyAssistant coincida con la apariencia de su terminal
...
MyAssistant admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/myassistant/themes/*.json` o `$XDG_CONFIG_HOME/myassistant/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.myassistant/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.myassistant/themes/*.json`
