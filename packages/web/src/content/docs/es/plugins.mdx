---
title: Complementos
description: Escriba sus propios complementos para extender MyAssistant.
---

Los complementos le permiten extender MyAssistant al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de MyAssistant.
...
- `.myassistant/plugins/` - Complementos a nivel de proyecto
- `~/.config/myassistant/plugins/` - Complementos globales
...
```json title="myassistant.json"
{
  ...
  "plugin": ["myassistant-helicone-session", "myassistant-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/myassistant/node_modules/`.
...
1. Configuración global (`~/.config/myassistant/myassistant.json`)
2. Configuración del proyecto (`myassistant.json`)
3. Directorio global de complementos (`~/.config/myassistant/plugins/`)
4. Directorio de complementos del proyecto (`.myassistant/plugins/`)
...
```json title=".myassistant/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
MyAssistant ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".myassistant/plugins/my-plugin.ts"
...
```js title=".myassistant/plugins/example.js"
...
- `client`: Un cliente SDK myassistant para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar myassistant.
...
```js title=".myassistant/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "myassistant"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio MyAssistant, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que myassistant lea archivos `.env`:

```javascript title=".myassistant/plugins/env-protection.js"
...
```javascript title=".myassistant/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a myassistant:

```ts title=".myassistant/plugins/custom-tools.ts"
import { type Plugin, tool } from "@myassistant-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que myassistant puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para myassistant junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://myassistant.ai/docs/sdk) para obtener más detalles.
...
```ts title=".myassistant/plugins/compaction.ts"
import type { Plugin } from "@myassistant-ai/plugin"
...
```ts title=".myassistant/plugins/custom-compaction.ts"
import type { Plugin } from "@myassistant-ai/plugin"
