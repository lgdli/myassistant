...
Las habilidades del agente permiten a MyAssistant descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
MyAssistant busca estas ubicaciones:

- Configuración del proyecto: `.myassistant/skills/<name>/SKILL.md`
- Configuración global: `~/.config/myassistant/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, MyAssistant sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.myassistant/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/myassistant/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.myassistant/skills/git-release/SKILL.md` así:
...
MyAssistant enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `myassistant.json`:
...
**Para agentes integrados** (en `myassistant.json`):
