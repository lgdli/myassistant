---
title: Proveedores
description: Usando cualquier proveedor LLM en MyAssistant.
---

import config from "../../../../config.mjs"
export const console = config.console

MyAssistant usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración MyAssistant.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu MyAssistant
configuración.
...
MyAssistant Zen es una lista de modelos proporcionados por el equipo MyAssistant que han sido
probado y verificado para funcionar bien con MyAssistant. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con MyAssistant Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione myassistant y diríjase a [myassistant.ai/auth](https://myassistant.ai/auth).
...
Funciona como cualquier otro proveedor en MyAssistant y su uso es completamente opcional.

---

## MyAssistant Go

MyAssistant Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de MyAssistant que han sido
probado y verificado para funcionar bien con MyAssistant.

1. Ejecute el comando `/connect` en TUI, seleccione `MyAssistant Go` y diríjase a [myassistant.ai/auth](https://myassistant.ai/zen).
...
Para usar Amazon Bedrock con MyAssistant:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta myassistant:
   ...
   Para una configuración persistente o específica del proyecto, utilice `myassistant.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en MyAssistant no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar myassistant para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que myassistant funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración myassistant.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `MyAssistant`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación myassistant.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
MyAssistant utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear MyAssistant
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `myassistant.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [myassistant-gitlab-auth](https://www.npmjs.com/package/myassistant-gitlab-auth).

##### Configuración

Personalizar a través de `myassistant.json`:
...
Para utilizar su suscripción GitHub Copilot con myassistant:
...
Para utilizar Google Vertex AI con MyAssistant:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta myassistant.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de myassistant, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [myassistant-helicone-session](https://github.com/H2Shami/myassistant-helicone-session) para registrar automáticamente cada conversación MyAssistant como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación MyAssistant enumerada como una sesión separada.
...
Puede configurar myassistant para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar myassistant para usar modelos locales a través de LM Studio.
...
Puedes configurar myassistant para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para MyAssistant. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/myassistant) para obtener más detalles.
:::
...
Para usar Ollama Cloud con MyAssistant:
...
3. Copie la clave API para usarla en MyAssistant.
...
6. **Importante**: Antes de usar modelos en la nube en MyAssistant, debe obtener la información del modelo localmente:
...
MyAssistant Zen es una lista de modelos probados y verificados proporcionada por el equipo MyAssistant. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>MyAssistant Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **MyAssistant Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave MyAssistant API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración myassistant.
...
5. También puedes personalizarlos a través de tu configuración myassistant. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con MyAssistant:
...
También puede personalizar modelos a través de su configuración myassistant. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con myassistant. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `myassistant.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a MyAssistant comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `myassistant auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración myassistant y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración myassistant.
