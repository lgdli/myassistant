---
title: Servidor
description: Interactuar con el servidor myassistant a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `myassistant serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente myassistant puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `myassistant.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `myassistant`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `myassistant serve` como a `myassistant web`.
...
Cuando ejecuta `myassistant`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor myassistant para interactuar con myassistant mediante programación.
:::

Esta arquitectura permite que myassistant admita múltiples clientes y le permite interactuar con myassistant mediante programación.

Puede ejecutar `myassistant serve` para iniciar un servidor independiente. Si tienes el
myassistant TUI ejecutándose, `myassistant serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos MyAssistant [IDE](/docs/ide).
...
El servidor myassistant expone las siguientes API.
