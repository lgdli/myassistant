...
Para depurar problemas con MyAssistant, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/myassistant/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\myassistant\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `myassistant --log-level DEBUG`.

---

## Almacenamiento

myassistant almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/myassistant/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\myassistant`
...
MyAssistant Desktop ejecuta un servidor MyAssistant local (el sidecar `myassistant-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `MyAssistant` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/myassistant/myassistant.jsonc` (o `~/.config/myassistant/myassistant.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/myassistant/myassistant.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\myassistant\myassistant.jsonc`
...
MyAssistant también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/myassistant/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\myassistant\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.myassistant/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que MyAssistant pueda reconstruirlo.

1. Salga de MyAssistant Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/myassistant`
- **Linux**: eliminar `~/.cache/myassistant` (o ejecutar `rm -rf ~/.cache/myassistant`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\myassistant`

3. Reinicie el escritorio MyAssistant.

---

### Solucionar problemas de conexión del servidor

MyAssistant Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `myassistant.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio MyAssistant requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de MyAssistant.

---

### Notificaciones que no se muestran

MyAssistant Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para MyAssistant en la configuración de su sistema operativo, y
...
1. Salga del escritorio MyAssistant.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio MyAssistant):

- `myassistant.settings.dat` (URL del servidor predeterminado de escritorio)
- `myassistant.global.dat` y `myassistant.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con MyAssistant:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/myassistant/issues**](https://github.com/anomalyco/myassistant/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**myassistant.ai/discord**](https://myassistant.ai/discord)
...
3. Asegúrese de tener la última versión con `myassistant upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `myassistant models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/myassistant
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\myassistant`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. myassistant instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/myassistant
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\myassistant`

2. Reinicie myassistant para reinstalar los paquetes del proveedor más recientes.

Esto obligará a myassistant a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
myassistant detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
