...
const deepLinkEvent = "myassistant:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__MYASSISTANT__ ??= {}
  const pending = window.__MYASSISTANT__.deepLinks ?? []
  window.__MYASSISTANT__.deepLinks = [...pending, ...urls]
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__MYASSISTANT__?.wsl) return undefined
    return commands.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__MYASSISTANT__?.wsl) return result
  ...
  return {
      ...
      await Promise.resolve()
        .then(() => {
          const notification = new Notification(title, {
            body: description ?? "",
            icon: "https://myassistant.ai/favicon-96x96-v3.png",
          })
    ...
    getWslEnabled: async () => {
      ...
      return window.__MYASSISTANT__!.wsl ?? false
    },
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("myassistant.global.dat").getItem("language")
