...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("myassistant-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("myassistant_lib=debug,myassistant_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("myassistant_lib=info,myassistant_desktop=info,sidecar=info")
        }
    });
