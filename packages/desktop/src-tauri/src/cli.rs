...
const CLI_INSTALL_DIR: &str = ".myassistant/bin";
const CLI_BINARY_NAME: &str = "myassistant";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("myassistant-install.sh");
...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.myassistant/bin/myassistant\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://myassistant.ai/install | bash -s -- --version {} --no-modify-path",
...
pub fn serve(
    ...
    let envs = [
        ("OPENCODE_SERVER_USERNAME", "myassistant".to_string()),
    ...
    .expect("Failed to spawn myassistant");
