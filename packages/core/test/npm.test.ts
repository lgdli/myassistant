...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@myassistant/acme")).toBe("@myassistant/acme")
    expect(Npm.sanitize("@myassistant/acme@1.0.0")).toBe("@myassistant/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/myassistant/acme.git"
    const expected = win ? "acme@git+https_//github.com/myassistant/acme.git" : spec
