...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.myassistant.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `myassistant.settings.dat` is special: it maps to the `myassistant.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"myassistant.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // myassistant.settings.dat → the electron settings store ("myassistant.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "myassistant.global.dat").
  const storeName = filename === "myassistant.settings.dat" ? "myassistant.settings" : filename
