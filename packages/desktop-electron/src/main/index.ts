...
process.env.MYASSISTANT_DISABLE_EMBEDDED_WEB_UI = "true"

const APP_NAMES: Record<string, string> = {
  dev: "MyAssistant Dev",
  beta: "MyAssistant Beta",
  prod: "MyAssistant",
}
...
import type { Server } from "virtual:myassistant-server"
...
function setupApp() {
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("myassistant://"))
    if (urls.length) {
  ...
  void app.whenReady().then(async () => {
    app.setAsDefaultProtocolClient("myassistant")
    registerRendererProtocol()
    setDockIcon()
    setupAutoUpdater()
    await initialize()
...
async function initialize() {
  ...
  const loadingTask = (async () => {
    ...
    if (needsMigration) {
      const { Database, JsonMigration } = await import("virtual:myassistant-server")
...
async function getSidecarPort() {
  const fromEnv = process.env.MYASSISTANT_PORT
  if (fromEnv) {
...
function sqliteFileExists() {
  ...
  return existsSync(join(base, "myassistant", "myassistant.db"))
}
