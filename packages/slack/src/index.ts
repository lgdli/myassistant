...
import { createOpencode, type ToolPart } from "@myassistant-ai/sdk"
...
console.log("🚀 Starting myassistant server...")
const myassistant = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await myassistant.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new myassistant session...")
    const { client, server } = myassistant
    ...
    console.log("✅ Created myassistant session:", createResult.data.id)
  ...
  console.log("📝 Sending to myassistant:", message.text)
