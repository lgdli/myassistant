...
import { createMyassistant } from "@myassistant-ai/sdk"
...
`)
  ...
  // Use a renamed API client to reflect the new branding
  const myassistant = await createMyassistant({ port: 0 })
  ...
  try {
    ...
    const session = await myassistant.client.session.create()
    const result = await myassistant.client.session
      .prompt({
